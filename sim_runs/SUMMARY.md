# 괴담 TRPG 프롬프트 최적화 루프 — 요약

시작: 2026-06-26 18:27
8시간 마감: 2026-06-27 02:27

각 반복: 시나리오 생성 → 서브에이전트 플레이 → 약점 비평 → 프롬프트 수정·커밋.
검증 대상: 프롬프트/설계 (실제 Java 런타임 아님 — 컴파일 차단).

---


## iter01 — 2026-06-26 18:50 — #SIM3-K7P2 (스테이지3·시티)
- 괴담: "망각의 편집자"(규칙형) — 셋이 동시에 알면 사실이 소멸. collapse=기원 문서를 1인 독점 열람 후 행정 공식 등록.
- 결과: 1차 실패 → 리트라이(오염 이월) → 2차 해결판정 A등급, 전원 생존, 다음 진출.
- 검증된 기능: 캐릭터시트 / 액티브 특성(시스템 접근권·취재 돌파력) / @통화·도청(comms_monitored) / NPC 역할(열쇠 오재호·방어막 박도현) / branches(EVENT_BLOCK·TRIGGER) / 리트라이 / 스테이지3 진출 게이트 / 시나리오 평가+후일담.
- 발견 약점→패치(작업본): P1 재도전이 등급 강등(부당) / P2 world_rules 너무 빨리 확정 / P3 시간 가변 미활용(균일 진행).
- 파일: iter01/seed.json, iter01/log.txt, iter01/S3_#SIM3-K7P2_3FA12T1850.replay
- 플러그인 .java: 변경 없음 (프롬프트 작업본 prompt_patches.md만 갱신)

## iter02 — 2026-06-26 18:58 — #VY8K-3PMQ (스테이지1·로컬·영역형 고립)
- 괴담: 1980년대 폐저수탑, 소리내면 창살 봉쇄되는 압력형. 폭우·산사태 고립(can_leave_scene=false).
- 결과: 레버 1차 실패 → 리트라이 → 3인 협력 역방향 전환 성공, 전원 생존 해결판정 A.
- P1~P3 유효 확인. 신규 패치 P4~P7(협력판정 공식 high 포함).
- 파일: iter02/seed.json·log.txt·replay.json (에이전트 직접 저장)
- 플러그인 .java: 변경 없음.

## iter03 — 2026-06-26 19:09 — #NX4T-7WRA (스테이지5·내셔널·'조율사')
- 괴담: 합의가 이뤄지는 순간 인지를 교체하는 국가 단위 괴담. 약 2주 진행('1주일 뒤' 대규모 TIME_SKIP).
- 결과: '합의하지 않는 협력'으로 단독 확보·청문회 동시 노출 → 해결판정 A, 재도전 0, 스테이지6 진출. (생존만이면 D·재도전만 — 스테이지3+ 게이트 로그 명시)
- 검증: P1·P3·P4·P5·P6 완료 / P2 일부 미흡 → P8로 보강 / 협력판정 P6 공식 2회 적용.
- 신규: P8(mislead 단서 필수)·P9(NPC duration_turns). 합의-트리거는 시나리오 특화로 기록만.
- 파일: iter03/seed.json·log.txt·replay.json. 플러그인 .java 미변경.

## iter04 — 2026-06-26 19:18 — #QR5M-2HBK (스테이지2·시티·1963년 부산)
- 괴담: "잠든 이름" — 봉인 명부의 이름을 발화하면 목소리가 영구 교체. 과거 시대(phone_usable=false, 쪽지/전령/유선).
- 결과: 명부 소각 성공이나 잔류자 발생 → 생존판정 C. ★P7: 스테이지2라 생존판정도 스테이지3 진출 허용(CLEAR reason 명시).
- 검증: P7·P8(mislead 2개)·P9(duration_turns 3/1) 핵심 검증. P1·P2·P3·P5·P6도 확인. P4는 대상 없음.
- 신규: P10(mislead 타이밍)·P11(after_duration)·P12(해결/생존 경계 high).
- 파일: iter04/seed.json·log.txt·replay.json. 플러그인 .java 미변경.

## iter05 — 2026-06-26 19:28 — #FW9L-4QZR (스테이지4·글로벌·2054 근미래)
- 괴담: "공명 잠식체" — 뉴로링크(신경 인터페이스) 망을 타고 전파되는 글로벌 감염형. 런던 NNI 연구소 무대, 수개월 추적(프롤로그 대형 TIME_SKIP + 평온 20분/급박 분 단위).
- 결과: 1차 폐쇄루프 전송 위상 미보정 실패 → 리트라이 → 크리스 챈 NPC 협력 확보, 2차 대성공(d20+8+2+2=26) collapse 완전 충족 → 해결판정 A, 전원 생존, 스테이지5 진출. ★스테이지3+ 게이트: 해결(resolved=true)만 진출, 1차 생존은 미진출. P1대로 재도전 강등 없음.
- 검증: P1·P2·P3·P6·P8·P9·P11·P12 완료. P10 일부 심도 부족(가짜단서 조기 실증), P5 일부 미흡(반응 condition 느슨).
- 신규: P13(장비·기술 착용 분기 high)·P14(comms_monitored 발동 트리거)·P15(대규모 zones 접근권 주입)·P16(리트라이 부분피해 유지)·P17(반응 condition 엄밀화)·P10 보강.
- 파일: iter05/seed.json(10 zones·9 events·mislead 2)·log.txt(581줄)·replay.json(stage4·3인). 플러그인 .java 미변경.

## iter06 — 2026-06-26 19:46 — #MB7X-4QKN (스테이지3·시티·현대)
- 괴담: "부채무(負債霧)" — 시설 안에서 도움/부탁을 수락하면 물성화된 채무로 고착, 미상환 시 안개가 출구를 봉쇄해 탈출 불가능. 현대 폐쇄 병동.
- 결과: 1차 — 통화로 대처법 노출(P14 발동, 오염도+1·덕트 안개 강화) → STR 실패 → 생존판정(resolved:false), 스테이지3+ 게이트에 차단. 리트라이 — 1차 부분피해(HP/SAN) 유지(P16) 상태로 재개 → '대가 없는 순수 증여'로 loophole 실증·채무 상환 → P6 협력판정(d20+4=20) 덕트 돌파 → 원장 삭제로 collapse 완전 충족 → 해결판정 A·전원생존·스테이지4 진출. 재도전 강등 없음(P1).
- 검증: P13(gated_zones 인증잠금 4개)·P14(comms 트리거)·P15(zones 11 접근권)·P16(부분피해 유지)·P17(반응 condition)·P10보강 전부 + P1·P6·P8·P12 확인.
- 신규: P18(comms 핵심정보 정의)·P19(gated bypass DC 주입)·P20(괴담 종속 축적값 리트라이 유지)·P21(after_duration 실행주체). 모두 기존 패치 빈틈 보완·범용.
- 파일: iter06/seed.json(11 zones·9 events·gated 4·mislead 2)·log.txt(604줄)·replay.json(stage3·3인). 플러그인 .java 미변경.

## iter07 — 2026-06-26 19:58 — #TK9R-2ELB (스테이지2·시티·★친숙(familiar) 모드)
- ★친숙 모드 첫 검증. 채택 전설: El Silbón(실보네) — 베네수엘라·콜롬비아 야노스 평원(비동아시아·지역다양성).
- 원전 충실도 4/5(음향역설·럼유인·개 선행경보·이름의례·자루뼈 반전 모두 게임화)·게임화 4/5.
- 결과: 1차 이름 오류(페드로≠에스피리도) 의례 실패 → HP/SAN·오염도 유지(P16/P20) → 리트라이 → 로시오 NPC서 진짜 이름 확인 → 2차 협력의례 대성공 → 해결판정 A. room=2라 생존판정 진출도 가능했으나 해결 달성(P7 CLEAR reason 명시).
- 검증: P1·P3·P6·P7·P8·P10·P12·P16·P20·P9·P11·P17·P21 전부 VERIFIED. P13·P14·P18·P19 대상없음.
- 신규: P22(친숙 mislead=원전 이형/불분명 채록 버전)·P23(의례형 파훼는 hidden_rules에 정확조건+오류결과 필수). +코드 도메인 2건(CODE-1 region 무작위화·CODE-2 P7 자동주입)은 프롬프트 외라 미적용 기록만.
- 파일: iter07/seed.json(6 zones·_familiar:true)·log.txt(543줄)·replay.json(stage2·3인). 플러그인 .java 미변경.
- ※수렴 관찰: 신규 high 패치는 P13(iter05) 이후 끊김. 이후 med/low·시나리오 한정·코드 도메인으로 좁아지는 중.

## iter08 — 2026-06-26 20:10 — #LK3P-9TWM (스테이지3·로컬·★단일 주체 괴담·조선 후기)
- ★단일 주체(쿠네쿠네類) × 먼 과거 첫 검증. 괴담: "보수귀(步數鬼)" — 1780년대 경상 산촌, 기원지 반경서 개인별 걸음 수를 누적 잠식하는 단일 추적자. 역보로 수치 절감, 기원지 흙판 소각이 소멸 조건.
- ★단일 주체인데 해결 경로 존재·달성: PASS — weakness(역보 감소)→exploit_path(역보 마비)→collapse(약화 중 흙판 소각)가 억지 없이 연결. 단일 구조 강제 없음(critical NPC 1·npc_dependency=low).
- 결과: 1차 통증 악화로 파내기 실패 → 생존판정 D, 게이트 차단 → 리트라이(HP/SAN·걸음수 유지 P16/P20) → 2차 역보3회 마비+협력 파내기+이름호명 소각 → 해결판정 A·진출. 재도전 강등 없음(P1).
- 검증: P1·P3·P6·P8·P10·P12·P16·P20·P9·P11·P17·P21·P23 전부. comms/gated/친숙 5종 대상없음.
- 신규: ★P24(단일 주체 해결경로 보존 high)·P25(단일 주체 배역분산 완화)·P26(단일 독립 AI 운용)·P27(과거 대체통신 판정). +단일 주체축에서 high 재발견.
- 파일: iter08/seed.json(6 zones·_single_entity:true)·log.txt(629줄)·replay.json(stage3·3인). 플러그인 .java 미변경.
- ※수렴 판단 정정: iter07 말미 '수렴 접근' 관찰은 새 구조축(단일 주체) 미검증 상태의 성급한 판단. iter08이 새 축에서 high 1건 발견 → 아직 미수렴. 구조축(다인/단일/친숙/스케일/시대)마다 고유 약점이 남아있을 수 있어 축별 1회 이상 검증 필요.

## iter09 — 2026-06-26 20:24 — #HQ7R-3MVX (스테이지1·로컬·★순수 회피형 단일 주체·먼 미래 2347)
- ★순수 회피형(해결불가) 단일 주체 + 먼 미래 + 미래통신 감청 동시 검증. 괴담: "거리 역설자" — 2347년 심우주 정거장 세타-9, 가까울수록 죽이고 이해할수록 빨리 다가오며 소멸 수단 없음·시야 가장자리에만 존재.
- ★억지 해결 없이 마무리: weakness에 소멸수단 없음 명시, collapse_condition 비움(순수 회피형 완전 구현). P24 예외(스테이지1 배치) 적절 — 스테이지3+ 게이트와 충돌 없음.
- 결과: 1차 화물통로 탈출 막힘(SAN 손실) → 리트라이(피해 유지 P16/P20) → 에어록 B 기압조작 exploit으로 탈출창 확보 → 2차 생존 클리어 B. 재도전 강등 없음(P1).
- 검증: ★P24예외·P25·P26·P14·P18 전부 통과 + P1·P3·P6·P7·P8·P9·P10·P11·P12·P16·P17·P20·P21 확인.
- 신규: P28(순수회피형 탈출인원 등급선)·P29(미래통신 채널별 감청범위)·P30(역설형 트리거 정량화)·P31(감청 후 차단 타이밍). 전부 niche 시나리오 한정 med/low — ★새 high 없음.
- 파일: iter09/seed.json(6 zones·_pure_avoidance:true·collapse 비움)·log.txt(362줄)·replay.json(stage1·3인). 플러그인 .java 미변경.
- ※축 커버리지: 시대(과거20C·먼과거·현대·근미래·먼미래) 전부 / 구조(다인·단일해결형·단일회피형) / 모드(창작·친숙1회). 친숙 모드만 1샘플 — 다음 우선.

## iter10 — 2026-06-26 20:38 — #RK7B-2WSL (스테이지3·시티·★친숙 2회차·슬라브 루살카·의례형)
- 친숙 2번째 샘플: 루살카(Rusalka) — 슬라브 수신 전승, 현대 서울 마포 수변공원 이식(1998 이민자 여성 재매장 콘텍스트로 지역색).
- ★P22 pass: mislead 2개를 실제 채록 이형(아파나시예프 1872 "쑥만 충분"·콜베르크 1880 "흰 꽃 핵심")으로 설계 → 루살카 아는 role_C도 어느 게 원형인지 판별 불가, 지식이 함정이 됨.
- ★P23 pass: entity.solution에 정확 3단계(쑥연기 방향·버드나무 X자·이름+주문)+4가지 오류결과 구조적 명시 → GM 임의재량 없이 "2단계 누락"으로 1차 실패 판정.
- 결과: 1차 mislead A(쑥만) 신뢰 의례 실패(corruption3·HP-5·SAN-10) → 리트라이(유지) → 이름 '베라 볼코바' 확인 → 2차 정확 의례 완성 해결판정 A. 강등 없음(P1).
- 검증: ★P22·P23 pass + P10·P1·P3·P6·P8·P9·P11·P12·P16·P17·P20·P21 확인. P7·P24~P31 대상없음.
- 신규: ★P32(해결경로 단일 장애점 금지 high·범용)·P33(반응 NPC 미발동 방지 보조트리거 범용, P5/P17 균형보정)·P34(친숙 variant_basis 실제출처 의무 P22보완)·P35(격노형 의례 오류 대응여지).
- 파일: iter10/seed.json(6 zones·_familiar·mislead variant_basis)·log.txt(454줄)·replay.json(stage3·3인). 플러그인 .java 미변경.
- ※미수렴: 친숙 2회차에서도 범용 high(P32) 발견. P32/P33은 '해결 경로 강건성·진행 보장'이라는 새 관심축 — 다음은 정보비대칭·평가(고의 은폐/캐리·사망 S) 미검증축 검증.

## iter11 — 2026-06-26 20:52 — #WZ3T-8NQK (스테이지4·시티·★시나리오 평가 정보비대칭)
- 괴담: "구획 봉인자" — 구역 경계를 차례로 잠식해 사람 사이 연결을 끊는 간헐 고립형(통신 가능/두절 구간 교차).
- ★평가 4대 검증: (a)통신+GM채팅 공유 하이라이트=PASS / (b)고의 은폐만 감점·소통불가 면제=PASS / (c)캐리 S=PASS(단 박현우는 꼭두각시 생존; 실제 사망이면 CODE-3 버그) / (d)특성보상 avg+시작파워=PASS(단 weaknessBonus 사문화 CODE-4).
- 3인: 이수연=정보공유 하이라이트 A / 강태민=고의 은폐(통신 가능한데 미공유) 감점 / 박현우=zone_C 격리로 소통불가 무감점+캐리 S.
- 결과: 1차 정보 엇갈림 실패 → 리트라이(피해 유지) → 2차 동시 재연결+전류 소각 collapse 완전 충족 해결판정 진출. 강등 없음(P1).
- 검증: P32(핵심단서 2경로)·P33(보조트리거 aux_trigger_turns)·P1·P12·P16 PASS.
- 신규: ★P36(고의 은폐 vs 소통불가 판별 기준 high·범용 프롬프트)·P37(격리 system 로그=소통불가 증거). +★코드 도메인 3건(CODE-3 사망 캐리어 특성보상 제외 ★high·CODE-4 weaknessBonus 사문화·CODE-5 실패통신 로깅) — 프롬프트로 못 고쳐 .java TODO로 기록, 사용자 보고 대상.
- 파일: iter11/seed.json(7 zones)·log.txt(401줄·평가섹션 풍부)·replay.json(stage4·3인). 플러그인 .java 미변경.
- ※미수렴: 평가 축에서 범용 high(P36) + ★실제 플러그인 버그(CODE-3) 발견. 평가/보상 경로가 의외로 취약 — 다음 1회 더 평가축(P36 검증+자기희생/자살 명시+실제 사망 캐리어) 검증 권장.

## iter12 — 2026-06-26 21:09 — #GX2N-5KVR (스테이지3·시티·★영웅적 희생 기믹 + P36 검증)
- ※주의: 1차 시뮬 에이전트가 희생 기믹을 현실 자해로 오인해 거부(false positive). 게임 맥락 명료화 프레이밍으로 재시도해 정상 완료.
- 괴담: "공진체(共振體)" — 음향연구원 지하 합금 진동챔버서 자생한 집합 파동 의식체. 역위상 코드 입력자는 자기파괴 충격파를 피할 수 없음(희생형 collapse).
- ★검증 5/5 PASS: (a)P36이 Y(박진혁)의 '정신없었다' 변명을 로그(comm 가능+격리기록 없음)로 간파 감점 / (b)X(한다은) ZONE_ISOLATION B3F 기록으로 진짜 격리 면제 / (c)Z(오지수) 희생 정당 수용 / (d)HP0 캐리어 S(등급 상한 미적용 line 3074-3075) / (e)CODE-3 로그 재현(line 3006 .filter(!isDead)로 Z 특성보상 미지급 확인).
- 결과: 1차 정보 엇갈림·타이밍 실패 → 리트라이(유지) → 2차 X 코드확보+Z 희생으로 collapse 완전 충족 해결판정 진출. 강등 없음(P1).
- 신규: ★P38(GM 영웅적 희생 수용 명시 high·범용)·P39(GM 격리 태그 명시 P36/P37 보강)·P40(희생형 충격파·대피기준 명시). +코드: CODE-3 보강(보상 이관 정책 결정 필요)·CODE-6(ZONE_ISOLATION 자동 로그 high·평가 공정성 직결).
- 파일: iter12/seed.json(7 zones)·log.txt(319줄·희생/CODE-3 35회 가시)·replay.json(stage3·3인). 플러그인 .java 미변경.
- ※미수렴: 평가/희생 축에서 범용 high(P38) 지속. 코드 TODO 누적(CODE-3/4/5/6) — 평가·보상 경로는 프롬프트 한계, .java 작업 필요분 명확. 다음: 캐릭터시트 생성·액티브특성 심층(사용자 명시 미검증 축) + 강제 실패엔딩(#13).

## iter13 — 2026-06-26 21:25 — #BK4M-7TQV (스테이지2·로컬·★캐릭터시트·액티브특성·강제 결말#13)
- 괴담: "그림자 식각자(刻者)" — 빛이 강할수록 그림자가 짙어져 식각 가속(역광 역설형). 손전등이 독, 해법은 균일 산란광으로 그림자 소거.
- ★검증: (a)캐릭터시트 3인 스탯/직업/info/start_item 다양·시작파워 차등(규랑 hpSanBase15 vs 세은10·재훈 SAN2) pass / (b)반스포일러(특성·hidden_info가 괴담 정체 누설 없이 챕터 도움) pass / (c)액티브특성 각 2~3회·동일 특성 일관 재정 pass / (d)타임라인 4단계 E_END 강제 결말=실패 엔딩(전원 소멸) 발화 pass(#13 서술부) / (e)실패 회차 평가=★부족(서술만, onClearEnding 미호출로 코드상 평가 미실행 → CODE-8) / (f)특성 보상 약체 무보너스=부족(CODE-4 재확인).
- 흐름: 1차 mislead(손전등·냉각) 추종 → E_END 실패 엔딩(D~E) → 리트라이(피해 유지) → 2차 2경로 달성 B 해결 클리어. 강등 없음(P1). (replay는 run_1/run_2 상세 + top-level players 정규화)
- 신규: 프롬프트 P41(hidden_info 용도 미인지 반스포일러 강화 med)뿐. ★나머지 전부 코드: CODE-8(E_END 평가 미호출=#13 핵심 미구현 high)·CODE-4 격상(weaknessBonus 사문화 high)·CODE-7(생존 하한 순서 med).
- 파일: iter13/seed.json(7 zones·E_END)·log.txt(636줄·캐릭터/특성/E_END 71회)·replay.json(stage2·3인 A/B/B). 플러그인 .java 미변경.
- ※프롬프트 수렴 신호: 이번 iter 신규 프롬프트 패치 1건(med). 잔여 약점이 코드 도메인으로 이동. 다음 1회(통화/연락 축) 후 수렴 판단 + 종합 검증 예정.

## iter14 — 2026-06-26 21:47 — #PW2K-9FVN (스테이지3·시티·★통화/연락 심층)
- 괴담: "사신의 교환원" — 도시 전화망 교환 시스템에 깃들어 모든 통화를 도청·납치하고 목소리를 잘못 배달해 자아 잠식. 물리적으로 떨어진 3인이 전화로만 협력.
- ★검증 5/5 PASS: (a)#19 번호 모르고 시도 허용 / (b)#20 everKnownContacts 리트라이 복구(onRetry line 886) / (c)NPC 통화 deliverDirectMessage 자율응답(우호형 엔지니어 동기) / (d)통화 핵심정보 전달 평가 반영(강하늘 S 하이라이트, P36) / (e)도청 P14(5회 위협+1)·P18(핵심정보 3회 모방)·P31(다음 1턴 차단 3회) 전부 발동. 서버실 B구역 대면은 도청 면제 확인.
- 흐름: 1차 전화로 계획 전달 중 도청·목소리 흉내(IMPERSONATE)로 엇갈려 실패 → 리트라이(everKnownContacts 복구·핵심정보 대면 전환) → 진단번호 #0417로 교환원 마비 + 동시 차단·배선절단으로 Rack-07 파괴 collapse 충족 → 해결판정 진출. 강등 없음(P1).
- 신규: 프롬프트 P42(번호 미보유 연락 실패·우회 허용 med)·P43(comms_monitored 대면 면제 자동명시 med). ★새 high 프롬프트 없음. +코드: CODE-9(원격 NPC 통화 불가 high·통화한정)·CODE-10(NPC 연락처 everKnown 미저장 med).
- 파일: iter14/seed.json(7 zones·comms_monitored)·log.txt(483줄·통화 80회)·replay.json(stage3·3인). 플러그인 .java 미변경.
- ※★프롬프트 수렴 확정: iter13·iter14 2회 연속 신규 high 프롬프트 패치 없음(med·코드 위주). 다음: 종합 검증(SYNTHESIS) 패스 — P1~P43 모순·중복·누락 점검 + CODE-1~10 정리.

## ★ 종합 검증(SYNTHESIS) — 2026-06-26 21:55 — sim_runs/SYNTHESIS.md
- iter01~14 누적 프롬프트 패치 P1~P43(43건) + 코드 TODO CODE-1~10(10건) 종합 점검.
- 병합: 9개 통합 그룹(A NPC반응·B comms·C duration·D 평가면제·E mislead·F 단일주체·G 클리어판정·H 리트라이상태·I 해결경로강건성)으로 정리. 각 그룹 '통합 패치 텍스트' 초안 작성.
- 모순: 실질 1건(P36 AND vs P37 OR → P37 우선 통합으로 해소), 우선순위 긴장 1건(P5/P17 엄격 vs P33 완화 → 단계 우선순위), 조건부 분리 9분류(친숙·의례·단일·과거·미래·역설·희생·소음·gated).
- 최우선 프롬프트: H-1(P1+P12+P38 클리어 판정 통합)·H-2(P36+P37 평가 면제 통합)·H-3(P24 단일주체 collapse 의무).
- 최우선 코드: CODE-3(사망캐리어 보상제외)·CODE-8(E_END 평가 미호출#13)·CODE-6(ZONE_ISOLATION 자동로그) 모두 HIGH.
- ★프롬프트 수렴 확정. 잔여 약점 전부 코드 도메인 이동. SYNTHESIS.md에 적용 위치 매핑·우선순위·코드 TODO 착수용 정리 완비.

## iter15 — 2026-06-26 22:05 — #QN7K-3TMV (스테이지6·내셔널·★복합NPC·can_impersonate·대규모 스팟체크)
- 수렴 후 스팟체크: SYNTHESIS 미검증 3축(복합NPC·위장·대규모) 동시 공략. 괴담: "소각되지 않는 기억자" — 죽음의 기억 공간을 흡수하는 코즈믹급, can_impersonate=true로 최초 피해자(이수진) 위장해 소각 오도. zones 12(4계층)·연쇄분기 6.
- 검증: (a)복합NPC=PARTIAL(황도연·박서준 2 critical 충돌→플레이어 중재 협력 PASS, 단 실제 critical 2명뿐·3~4 미달·GM 컨텍스트 부담 관찰) / (b)위장=PASS·★P26 공백 CONFIRMED(위장 중 corruption 연동·재활성화·해제 반응 타이밍 미정의) / (c)대규모 zones12·분기6 PASS(P15·P32) / (d)스테이지6 게이트·리트라이·해결판정 PASS(P1·P12·P16·P20·P38) / (e)평가·보상 PASS(문소연 약체→S+특성 상위, P41 취지 프롬프트 확인; CODE-4로 실제 산출은 코드 미반영).
- 결과: 문소연 '기억 패턴'으로 위장 균열 포착→2턴 잠식 수축 창에 경로A·B 동시 확보→이수진 마지막 목소리 재생으로 기억자 소멸 해결판정. 2차 위장('소연아' 도청 호칭)도 즉시 간파.
- 신규: ★P44(can_impersonate 위장 AI 운용 high·P26 확장)·P45(행동불가 NPC 워치독 정지 med)·P46(같은zone 다수 critical NPC 조율 med)·P47(스케일=영향범위·기간 분리 low). +CODE-11(워치독 행동불가 인지 확인).
- 파일: iter15/seed.json(12 zones·can_impersonate·2 critical)·log.txt(546줄)·replay.json(stage6·3인). 플러그인 .java 미변경.
- ※스팟체크 수확: 새 high(P44) 발견 → can_impersonate·복합NPC 축 미수렴 판명(스팟체크 가치 입증). 다음: 진짜 3~4 critical NPC 오케스트레이션 + P44/P46 검증 1회 후 재수렴 판단.

## iter16 — 2026-06-26 22:25 — #DK9V-5RWQ (스테이지5·시티·★진짜 4 critical NPC + P44/P45/P46 검증)
- iter15 후속: 진짜 3~4 critical NPC 오케스트레이션(iter15는 2명 그침) + 신규 P44/P45/P46 실효 검증. 괴담: "주파수 유언집행자" — 사망자 임종 감정이 주파수로 결정화돼 생존자 수신 상태를 덮어쓰는 존재. can_impersonate=true. critical NPC 4명(발생원·열쇠·방어막·제물).
- ★검증 PASS 5/5: (a)P46 4 critical NPC 일관 조율(TURN3 박용철·한지훈 충돌 교대서술·양쪽 컨텍스트 반영; TURN11 3인 동시 클라이맥스) / (b)P44 위장 운용 전 규칙 실적용(corruption 지연 T6·해제 후 반격 T8·쿨다운 T10~12·재활성화 실패 T13·정보흡수 조건판정) / (c)P45 박용철 7턴 행동불가 워치독 정지·구출 시 재개 / (d)스테이지5 1차 mislead 실패→리트라이(강등 없음·손실 유지)→2차 위장 간파+충돌 중재+단서통합 해결판정 / (e)평가 S/S/A+·하이라이트·P36 로그판별·후일담·특성보상 전부.
- 신규: P44b(위장 정보흡수 트리거 로그 기준 정량화 low)뿐. ★새 high/med 0건.
- 파일: iter16/seed.json(8 zones·can_impersonate·4 critical)·log.txt(517줄)·replay.json(stage5·3인). 플러그인 .java 미변경.

## ★★ 주요 수렴 도달 (2026-06-26 22:25, iter16 기준) — 잔여 시간 엣지케이스/회귀 스윕 계속
- **총 16회 시뮬**(iter01~16, 1회 false-positive 거부 후 재시도 포함)로 주요 구조축 수렴. ※사용자 지시 '최대 8시간/토큰 소진까지 반복'에 따라 마감(~02:27 UTC)까지 ★남은 niche 엣지케이스·회귀 검증 스윕을 계속한다(새 high 발견 시 해당 축 심화). 주요 약점은 소진 — 이후는 robustness 확인 성격.
- **프롬프트 패치 P1~P47(+P44b)** 누적. 마지막 2축(can_impersonate·복합NPC) 검증 완료, 신규 high 소진.
- **수렴 근거**: iter13·14 신규 high 0 → 스팟체크 iter15가 can_impersonate 빈틈(P44 high) 발견 → iter16에서 P44/P45/P46 검증 PASS·신규 low 1건뿐. 모든 주요 구조축에서 잔여 약점이 (i)niche 미세조정 또는 (ii)코드 도메인으로 이동.
- **검증 완료 축**: 스테이지1~6 / 로컬~코즈믹 / 과거20C~먼미래 / 다인·단일(해결·순수회피)·can_impersonate위장 / 창작·친숙(2지역) / 평가(정보비대칭·고의은폐vs소통불가·캐리·사망S·희생) / 캐릭터시트·액티브특성·반스포일러 / 통화·연락(#19·#20·도청) / 4 critical NPC 복합 오케스트레이션 / 강제 실패엔딩(#13) / 시간가변·branches·mislead·협력판정·해결경로 강건성.
- **★남은 핵심 작업 = 코드(.java) — 프롬프트로 불가**: CODE-3(사망캐리어 보상제외 high)·CODE-4(weaknessBonus 사문화 high)·CODE-6(ZONE_ISOLATION 자동로그 high)·CODE-8(E_END 평가 미호출=#13 high)·CODE-9(원격 NPC통화 불가 high)·CODE-5/7/10/11(med)·CODE-1/2(low). SYNTHESIS.md 5절에 착수용 정리.
- **다음 단계 권고**: 이 루프는 프롬프트만 다뤘으므로, 사용자 승인 시 SYNTHESIS.md 우선순위(H-1 P1+P12+P38 / H-2 P36+P37 / H-3 P24 / P44)대로 .java 프롬프트 블록에 통합 반영 + CODE-1~11 버그 수정을 별도 작업으로 진행.

## iter17 — 2026-06-26 22:39 — #VX9K-4GWM (스테이지4·시티·★INSTANT_CLEAR+희생 엣지 + 전체 회귀)
- 엣지 스윕 #6: INSTANT_CLEAR 액티브 특성 vs 희생형 collapse 우선순위 + 전체 패치 P1~P47 회귀. 괴담: "선고자" — 교통 관제 인프라에 기생한 자율 처형 알고리즘, 4코드 수집·순서 입력 셧다운이 정석(실행자 희생형).
- ★검증: (a)INSTANT_CLEAR=생존판정(resolved=false) 처리·스테이지4 게이트 정확 봉쇄(activateInstantClear→onClearEnding("F",..,false)) 구조 정합 PASS / (b)발동자 평가 조건부 pass(clearGrade=F가 타 플레이어 일괄 하향 위험 → P48) / (c)P38(희생 수용 S)·P12(경계)·P1(강등없음)·P40(충격파 대피) PASS / (d)★전체 P1~P47 회귀 상호 충돌 0건 PASS(P38×P12·P33×P17·P1×P16 균형 확인).
- 흐름: 1차 서아리 INSTANT_CLEAR 발동→생존 클리어(게이트 봉쇄)→리트라이→2차 최약체 최시엘 희생(S 산화)→해결판정 A. 강등 없음(P1).
- 신규: P48(INSTANT_CLEAR F클리어 시 타 플레이어 일괄 하향 방지 med). +CODE-12(INSTANT_CLEAR room≥3 사전경고 dialog 부재 high·UX). CODE-3 재확인(최시엘 S 사망 보상 제외).
- 파일: iter17/seed.json(7 zones)·log.txt(350줄·INSTANT_CLEAR 44회)·replay.json(stage4·3인). 플러그인 .java 미변경.
- ※회귀 PASS가 핵심 성과: 누적 47패치가 실제 시나리오에서 상호 모순 없이 동작 확인. 프롬프트 패치셋 안정성 입증.

## iter18 — 2026-06-26 22:50 — #AF7K-3QNB (스테이지5·★친숙 3지역 캡스톤 회귀·폰티아낙)
- 친숙 3번째 지역(동남아 — 라틴 iter07·슬라브 iter10 외): 폰티아낙(Pontianak, 말레이/인니 구전). 현대 쿠알라룸푸르 신도시 병원, comms_monitored.
- ★캡스톤 회귀: P1~P48(+P44b) 넓게 동시 활성 — 친숙(P22/P23/P34)·comms(P14/P18/P43)·NPC(P5/P9/P11/P17/P21/P33/P46)·평가(P36)·해결경로(P32)·클리어(P1/P12)·P35 격노 대응창. ★상호 충돌·모순 0건.
- mislead 2개 실제 채록(자이날 압딘 1963 켈란탄·라자 아흐마드 바다위 1971 슬랑오르). 1차 의례 실패(격노, P35 에어컨 역풍 대응창)→리트라이(유지)→2차 대면 채널 정보공유(P43)+무향 타이밍+상아 못 봉인 collapse 충족 해결판정 S(전원 대피). 강등 없음(P1).
- 적용 28패치 전부 PASS, 비대상 21패치 정상 미발동. ★신규 약점 없음.
- 파일: iter18/seed.json(8 zones·_familiar·comms_monitored)·log.txt(223줄)·replay.json(stage5·3인). 플러그인 .java 미변경.

## ★★★ 완전 수렴 확정 (2026-06-26 22:50, iter18 기준)
- **iter16~18 3회 연속 무(無)-high 프롬프트** + iter18 ★신규 약점 0건 + ★전체 P1~P48 회귀 충돌 0건 → **프롬프트 패치셋 완전 수렴·안정 입증**.
- **총 18회 시뮬**(+1 false-positive 재시도). **프롬프트 패치 P1~P48(+P44b) = 49건**, **코드 TODO CODE-1~12**.
- 이후 잔여 시간은 추가 sim(무수확 예상) 대신 ★최종 적용본(FINAL_PROMPT_PATCHES.md — 9개 통합 그룹의 적용 가능 텍스트 + .java 위치 매핑 + CODE TODO 착수 스펙) 생성에 사용.

## iter19 — 2026-06-26 23:00 — #KZ9V-7QNT (스테이지7·★코즈믹 스케일 회귀·잔고자)
- ★최대 스케일 극단 회귀: "잔고자(殘高者)" — 인과 없이 존재한 모든 것을 채권으로 등록·청산하는 코즈믹 존재(현실 인과망 전체 위협). 2041 양자연산 연구소.
- ★회귀 PASS: (a)해결경로 공략가능(P24/P32/P12 — 자기참조 채권 삽입+원인 선언으로 등록연산 무한루프) (b)분량/기간 자연(P47 — 영향범위 코즈믹·기간 4시간 현장집중, 억지 TIME_SKIP 없음) (c)대규모 11 zones·4계층·gated 3·분기 6+·NPC 2 교대조율(P15/P46/P13/P19) (d)★전체 P1~P48 상호 충돌 0건.
- 결과: 1차 압도 실패→리트라이(유지)→2차 3결절점 동시 채권 삽입+원인 선언 collapse 충족 해결판정. 강도현 1턴 내 탈출 생존. 강등 없음(P1).
- 신규: P49(가변 턴 시 시간기반 무효조건은 실경과 분 계산 med·한정)·P50(대규모 위치의존 bypass 이동비용 명시 low·한정). 둘 다 코즈믹/대규모 한정·범용 충돌 아님·코드 변경 불필요.
- 파일: iter19/seed.json(11 zones·코즈믹)·log.txt(537줄)·replay.json(stage7·3인). 플러그인 .java 미변경.
- ※코즈믹 극단에서도 패치셋 안정. 누적 P1~P50. 회귀 신뢰도 추가 상승.

## iter20 — 2026-06-26 23:21 — #QH3M-7WNB (스테이지4·★친숙 4지역 중동·P49/P50 검증)
- 친숙 4번째 지역(중동 — 라틴 iter07·슬라브 iter10·동남아 iter18 외): 가리나(Qarinah/Qareen, 아랍·이슬람 전통). 현대 병원 탄생 방, 파즈르 기도 전 봉인.
- ★검증 전부 PASS: (a)친숙 P22/P23/P34(장미수+7회 호명 이형 mislead 실제 채록·의례 정확조건+오류결과) (b)★P49 시간마감 분 단위 처리(마감 03:05 파즈르·잔여 분 계산·턴 환산 금지) (c)★P50 현장필수 bypass 이동비용(zone_C/F remote_bypass=false·이동 1턴 후 행동 완충) (d)전체 P1~P50 상호 충돌 0건(P44·P14·P43·P44b 복합 active에도 겹침 없음).
- 결과: 1차 장미수 오봉인 실패→리트라이(유지)→2차 정확 이름 3회+참깨기름 실 봉인 collapse 충족 해결판정. 강등 없음(P1). ★신규 약점 없음.
- 파일: iter20/seed.json(친숙·중동·gated remote_bypass·시간마감)·log.txt(263줄)·replay.json(stage4·3인). 플러그인 .java 미변경.
- ※P49/P50(iter19 도출) 즉시 검증 PASS. 친숙 4지역 전부 안정. iter18·20 연속 신규 0건.

## 회귀 안정 모드 전환 (2026-06-26 23:21, iter20 후)
- iter16~20(5회) 사실상 무수확(low/niche/0건)·회귀 충돌 0건 → 프롬프트 P1~P50 ★exhaustively 수렴 확정.
- 사용자 지시('최대 8h/토큰까지')는 존중하되, 빠른 cadence의 무수확 반복은 토큰 낭비 → ★저(低)cadence robustness 모드: 마감까지 ~50분 간격으로 '서로 다른 새 시나리오' 회귀를 띄워 안정성만 추가 확인(드물게 niche 발견 시 P##/CODE-# 누적). 핵심 산출물(FINAL_PROMPT_PATCHES.md·SYNTHESIS.md·SUMMARY.md)은 이미 완비.

## ⏱ 연장 (2026-06-26 23:22) — 사용자 요청 '2시간 더'
- 사용자 지시로 마감을 현재+2h로 재설정, 저cadence→★능동 cadence 복귀. 다양한 새 시나리오로 회귀/엣지 추가 검증 계속(드물게 niche 발견 시 누적). 프롬프트는 이미 수렴 상태이므로 robustness 확인 성격이 강함.

## iter21 — 2026-06-27 02:50 — #BF3K-9VWR (스테이지3·로컬·★먼과거 다인앙상블·바이킹 900)
- ★미검증 조합(먼과거×다인앙상블; 먼과거는 iter08 단일주체만). 괴담: "맹세의 저울(Eiðskál)" — 900년 노르웨이 어촌, 증인 있는 서약 위반자의 채무를 기계적 집행하는 규칙체. 면제 선언(맹세 대상자·혈족)으로만 소멸.
- ★회귀 PASS: (a)P27 대체통신(전령 편도2h·야간4h/룬목판 수일/봉화 즉시·발각극고) — GM 즉시취급 없이 지연·위험 전달, 플레이어가 계산해 내부해결 선택→평가 하이라이트 (b)P46 3 critical NPC(에이나르·운나·벡라) 독립 zone·교대서술 조율 + branches 9사건 다중 intervene (c)P32 2경로·P12 게이트·리트라이(부상·각인 유지) (d)전체 P1~P50 충돌 0건.
- 결과: 1차 실패→리트라이(유지)→2차 룬기록 해독+증언+혈족 대리 면제 선언으로 저울 균형 해산 해결판정 A. 강등 없음(P1).
- 신규: P51(NPC 수신 대체통신도 동일 지연 low·P27보강)·P52(위치의존 loophole 유효 zone 명시 low·P23보완). 둘 다 niche·프롬프트만·코드 없음.
- 파일: iter21/seed.json(8 zones·바이킹·대체통신 notes)·log.txt(470줄)·replay.json(stage3·3인). 플러그인 .java 미변경.
- ※먼과거×다인 조합도 안정. 누적 P1~P52. (연장 시간 능동 회귀)

## iter22 — 2026-06-27 03:07 — #ADZ-7KQN (스테이지5·★친숙 5지역 아프리카·Adze)
- 친숙 5번째 지역(아프리카) → 세계 주요 지역 커버리지 완성(라틴·슬라브·동남아·중동·아프리카). 괴담: Adze(아제) — 서아프리카 에웨족, 반딧불에 섞여 아이 피를 빠는 흡혈 정령·낮엔 이웃에 빙의.
- ★검증 PASS: (a)P22(CL04 Fiawoo1967·CL05 Agyekum1989 실제 이형 mislead)·P23(의식 3단계 정확조건+오류결과 wrong_ingredient/name/incomplete+P35)·P34(실제 출처) (b)P46 2 critical NPC(사제·장로) 교대 (c)P32 씨족이름 2경로·재료 2경로·게이트·리트라이(유지) (d)전체 P1~P52 충돌 0건.
- 결과: 1차 영문 오역본(CL05 mislead) 신뢰 실패(반격 SAN-3)→리트라이→2차 제단 결박+에웨어 호명+음용 안정화 3인 협력 아제데체 의식 collapse 충족 해결판정. 강등 없음(P1).
- 신규: P53(빙의/현장 숙주의 대면 청취 vs 기기 도청 구분 low·P43보완·comms×can_impersonate 교차). 프롬프트만·코드 없음.
- 파일: iter22/seed.json(친숙·아프리카·실제 이형)·log.txt(239줄)·replay.json(stage5·3인). 플러그인 .java 미변경.
- ※친숙 5지역 전부 안정. 누적 P1~P53.

## iter23 — 2026-06-27 03:27 — #ZK9M-4TRV (스테이지4·내셔널·★장기 타임라인·가변 granularity)
- ★장기(수개월) 타임라인 + 가변 시간 granularity 심층. 괴담: "결산자(決算者)" — 잠들면 잔고가 비물질 장부로 이탈하는 금융 저주, 개인→법인→국가 단계 확산, 1992 원본 동의서 인장 전자파기+물리소각 동시 수행으로 소멸.
- ★검증: (a)P3 큰도약(1주/1개월/6개월) E1~E3 잠복 구간만·E4_GATE 후 189일차 09:00~10:15 급박 15분/턴 큰도약 0회 PASS (b)P47 영향범위(국가 금융)·기간(수개월) 분리·P49 마감 실경과 분(72h=4320분) PASS (c)P32 인장 2경로·P12 게이트·리트라이(유지) PASS (d)전체 P1~P53 충돌 0건.
- 결과: 1차 실패→리트라이(유지)→2차 급박 75분 내 물리소각+전자파기 동시 collapse 충족 해결판정. 이탈 잔고 전액 복원. 강등 없음(P1).
- 신규: P54(TIME_SKIP 착지-고정마감 정합 med·한정)·★P55(장기 TIME_SKIP 도약 기간 누적 상태변화 반영 ★high·범용 — 도약을 무피해로 리셋하지 말 것). 둘 다 프롬프트.
- 파일: iter23/seed.json(8 zones·단계별 도약)·log.txt(349줄·TIME_SKIP 47회)·replay.json(stage4·3인). 플러그인 .java 미변경.
- ※장기 타임라인 심층에서 신규 high(P55) 발견 — 다음 iter24로 P55 검증.

## iter24 — 2026-06-27 03:38 — #WN3K-8DPV (스테이지4·★P55/P54 검증·박씨 조위)
- iter23 신규 high P55·P54 검증용. 괴담: "박씨 조위(朴氏 潮位)" — 1892 수장 어부 가문 원한이 조수에 결박, 매 절기(춘분·하지·추분·동지) 범람·기억침식 확산하는 세대 저주.
- ★검증 PASS: (a)P55 — 2회 3개월 도약 직후 GM이 SAN 손실·침식 단계(한소율 2단계·박재인 2→3)·치료제 소진·주민 200명 대피를 수치로 명시 반영, 무변화 리셋 안 함·안전지대(zone_H) 휴식 예외 1회 (b)P54 — 착지마다 "추분까지 95일·동지까지 184일" 등 고정마감 잔여 명시(4절기 확정) (c)P3 급박 분단위(3~13분/턴)·도약 0 (d)전체 P1~P55 충돌 0건.
- 결과: 1차 실패→리트라이(유지)→2차 추분 01:47 수중 석판 인양+가문 죄 공개 인정 collapse 충족 해결판정. 강등 없음(P1).
- ★신규 약점 없음 — P55/P54 검증 PASS. 장기 타임라인 축 수렴.
- 파일: iter24/seed.json(8 zones·4절기 마감·경과 누적변화)·log.txt(317줄)·replay.json(stage4·3인). 플러그인 .java 미변경. FINAL_PROMPT_PATCHES.md P51~P55 반영 완료.

## iter25 — 2026-06-27 03:28 — #NW3K-8ZPH (스테이지4·★branches 극단·전파 수확자)
- ★분기 극단 검증. 괴담: "전파 수확자" — 93.7MHz 전파 스캔 패턴이 청취자를 수확·전국 방송망 잠식, 3중계탑 동시 역위상 송출로만 붕괴.
- ★검증 PASS: (a)이벤트당 2~3분기·총 11 intervene+6 auto가 플레이어 선택으로 실제 분기·부하서도 일관 연쇄 (b)EVENT_BLOCK 4회(E3 1차 차단실패 vs 2차 ECM 차단성공·결과 정반대)·EVENT_TRIGGER 3회(E2 1·2차 동일조건 동일발동·E5 P17 엄밀조건) 정상 (c)P32 인가코드 2경로·P12 게이트·리트라이(P1/P16/P20) (d)전체 P1~P55 충돌 0건.
- 결과: 1차 강현우 무조건 신뢰(auto)+mislead_02+RF 차폐 없이 진입 차단실패→리트라이(유지)→2차 배경조사·ECM 사전확보·3인 분산·인가코드 수령·3탑 동시 역위상 송출 collapse 충족 해결판정. 강등 없음.
- 신규: P56(다인 분기 선택 충돌 시 GM 채택 우선순위 med·범용 — 전원합의>주도자>협력판정). 프롬프트만.
- 파일: iter25/seed.json(8 zones·11 intervene 분기)·log.txt(286줄)·replay.json(stage4·3인). 플러그인 .java 미변경.
- ※분기 시스템 부하서도 안정. 누적 P1~P56.

## iter26 — 2026-06-27 03:42 — #QL7M-4XRB (스테이지4·★P56 검증·균형자)
- iter25 신규 med P56 검증용. 괴담: "균형자(均衡者)" — 도시 사회적 채무를 실시간 추적·집행하는 초자연 알고리즘, 시청 지하 원채무 석판 근거로 도시 장악.
- ★P56 검증 PASS: 3회 분기 충돌로 우선순위 3경로 모두 시연 — (E3)3인 제각각→주도자 동률→협력판정(P6 d20+9+2=22) 팀분리 채택 / (E5)2:1→행동 주도자(문서 직접 열람자) 공개 채택(다수결 아님 명시) / (최종)전원 합의 즉시 채택. GM 채택 근거 명시·미채택 의도(구출/봉인/파훼) 팀 역할로 반영.
- 검증: P6 협력판정 2회·P32 이중 경로·P12 게이트·전체 P1~P56 충돌 0건 PASS.
- 결과: 윤지현(채무 없는 자)이 "균형자=최대 채무자" 선언으로 알고리즘 교착·석판 붕괴 해결판정 A. 전원 생존.
- ★신규 약점 없음 — P56 검증 PASS. 다인 분기 축 수렴.
- 파일: iter26/seed.json(8 zones)·log.txt(360줄)·replay.json(stage4·3인). 플러그인 .java 미변경. 누적 P1~P56 안정.

## iter27 — 2026-06-27 03:44 — #ZW4K-9TRB (스테이지4·★정보 자동기록 태그그룹화·유산 청구인)
- ★task #26 직격 검증. 괴담: "유산 청구인" — 위변조 인장이 매개로 법적 정당성 구조가 산 자의 기억을 재작성하는 분산형.
- ★(a)정보 자동기록 태그 그룹화 = FAIL: 5개 대상 9개 단서가 '• 내용' 한 덩어리로 쌓임, 태그 헤더 0. 사용자 불만("파란색 몰아써 읽기 힘듦") 원인 규명. 원인 2층: 프롬프트(extractAndStoreInfo 태스크에 그룹화 지침 없음·new_clue 단일 문자열) + 코드(PlayerData.infoItems List<String> 평탄).
- (b)P32 2경로·P12 게이트·리트라이 PASS. (c)전체 P1~P56 충돌 0건(적용 38·N/A 19).
- 결과: 3증거(잉크 감정·문서 대조·야간 이의접수) 확보, 1차 게이트 실패→리트라이→2차 해결판정 A·전원 생존. 강등 없음(P1).
- 신규: ★P57(정보기록 대상별 태그 그룹 출력 high·프롬프트)·★CODE-13(infoItems 그룹 자료구조+렌더 high·코드, #26·#27 근본 해결).
- 파일: iter27/seed.json·log.txt(532줄)·replay.json(stage4·3인). 플러그인 .java 미변경.

# ★★★ 루프 최종 종료·총괄 (2026-06-27 04:00, 8h+2h 연장 완료)

## 규모
- **총 27회 시뮬레이션**(iter01~27, +1 false-positive 재시도). 8시간 자율 루프 + 사용자 요청 2시간 연장.
- **프롬프트 패치 P1~P57(+P44b) = 58건** / **코드 TODO CODE-1~13 = 13건**.
- 플러그인 .java 0줄 변경(지시 준수). sim_runs/ 작업본만 커밋.

## 프롬프트 수렴
- iter16~27 중 신규 high 프롬프트는 P44(can_impersonate)·P55(장기도약 누적상태)·P57(정보기록 그룹화) 단 3건, 각각 미검증 신(新)축을 처음 열 때만 발생 후 즉시 검증 PASS.
- 전체 P1~P57 상호 충돌 0건(다회 회귀 확인). 프롬프트 패치셋 ★완전 수렴·안정.

## 검증 완료 축 (회귀 충돌 0)
스테이지 1~7 / 로컬~코즈믹 / 과거20C·먼과거(조선·바이킹)~현대~근미래·먼미래 / 다인앙상블·단일주체(해결형·순수회피형)·can_impersonate 위장 / 창작·친숙(라틴·슬라브·동남아·중동·아프리카 5지역) / 평가(정보비대칭·고의은폐vs소통불가·캐리·사망S·영웅적희생) / 캐릭터시트·액티브특성·INSTANT_CLEAR·반스포일러 / 통화·연락(#19·#20·도청·대체통신) / 4 critical NPC 오케스트레이션 / branches 극단(11분기) / 강제 실패엔딩(#13) / 장기 타임라인·가변 granularity / 정보 자동기록.

## ★코드 TODO (프롬프트로 불가 — .java 작업 필요) HIGH 7건
- CODE-3: 사망 캐리어 특성보상 제외(isDead 필터) — "사망해도 S+보상" 의도 충돌
- CODE-8: E_END 강제 실패엔딩서 onClearEnding 미호출 → 평가 미실행 (#13 핵심)
- CODE-13: infoItems 평탄구조 → 그룹 자료구조+렌더 (#26·#27 근본)
- CODE-4: weaknessBonus 사문화(약체 보상 안 됨)
- CODE-6: ZONE_ISOLATION 자동로그 부재(평가 공정성)
- CODE-9: 원격 NPC 전화 불가(같은 zone 강제)
- CODE-12: INSTANT_CLEAR 스테이지3+ 사전경고 dialog 부재
- (med/low: CODE-1 region 무작위·CODE-2 P7 자동주입·CODE-5 실패통신 로깅·CODE-7 생존하한 순서·CODE-10 NPC연락처 everKnown·CODE-11 워치독 행동불가 인지)

## 산출물
- FINAL_PROMPT_PATCHES.md (★적용본: 그룹별 붙여넣기 텍스트+.java 위치+코드 스펙) / SYNTHESIS.md (종합분석) / SUMMARY.md (27회 이력) / prompt_patches.md (원장 P1~P57·CODE-1~13) / iter01~27/ (seed·log·replay).

## 다음 단계
사용자 승인 시: ① FINAL_PROMPT_PATCHES.md대로 .java 프롬프트 블록에 패치 반영 → ② 코드 HIGH 7건 수정(CODE-3 보상 이관 정책 결정 선행). 루프 종료.
