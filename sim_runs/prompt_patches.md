...
- [수정안] "서술은 플레이어가 쉽게 이해할 수준의 어휘로 쓴다. 어려운 한자어·전문 용어·고유 명칭이 불가피하면 그 단어 뒤에 *를 붙이고 ★다음 줄에 '*주석 : <쉬운 풀이>'를 단다. 형식:
  ```
  잔고자가 인과(因果)*를 청산하기 시작했다.
  *주석 : 인과 = 원인과 결과
  ```
  여러 어려운 단어가 한 서술에 있으면 각 줄 뒤 또는 문단 끝에 *주석을 모아 단다. 괴담 정체 공개(P61) 시 이름이 어려운 한자어면 그 뜻 풀이도 *주석으로."
