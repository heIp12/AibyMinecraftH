...
- ★CODE-3 (iter11 WEAK-02, ★high) ★결정 확정(2026-06-27): TRPGGameManager grantClearTraitRewards의 isDead 필터(약 line 3006)를 ★제거 — 스테이지 클리어 시 ★사망 여부 무관 전원에게 보상 무조건 지급(다음 스테이지=전원 부활이므로 죽은 자도 돌아와 보상 수령). 별도 이관 정책 불필요. ※.java 수정: `.filter(!isDead)` 삭제(클리어 보상 경로 한정).
...
- CODE-3 보강 (iter12) → ★결정으로 단순화: 이관 정책 불요. '클리어 시 사망 무관 무조건 지급'으로 확정(위 CODE-3 참조).
...
## ★ 사용자 교정 8 (post-loop, 2026-06-27) — P70 (#16 재정의: 괴담 연루 배역 허용·본인 미인지)

### P70. 괴담 연루 배역 허용 — 씌인 피해자/괴담 주체, 단 본인은 모름 (★high, 범용 — #16 재정의)
- [대상] GdamGenerator 배역(roles) 설계 + GM_SYSTEM_BASE 반스포일러 운용 (기존 #16 '괴담측 배역 금지' 개념을 ★허용+은닉으로 교정)
- [정정] #16은 '괴담 측 배역 생성 금지'가 아니다. 괴담 연루 배역을 ★허용하되 ★본인이 그 사실을 모르는 상태로 설계한다.
- [수정안 — 두 유형]
  - ① ★괴담에 씌인 피해자 배역: 가능. 이 경우 그 플레이어는 괴담에 ★큰 접근 권한을 가진 '키'가 될 수 있다(특수 단서·약점·구역 접근). 단 씌었다는 사실은 hidden_info에 숨김.
  - ② ★플레이어 자체가 괴담의 주체(발생원) 배역: 가능. ★본인은 모르는 상태로 시작해, 플레이 중 단서로 깨닫고 ★자기 자신을 해결해야 한다(자기 인지·정화·자기희생 등).
- [공통 원칙]
  - 연루 사실은 ★시작 시 본인도 모름 — hidden_info에 두고 GM은 즉시 노출 금지, 단서로 점진 공개(반스포일러 P41·발견형 공개 P61과 정합).
  - ★해결 가능성 보장(P24/P67): 주체·씌인 배역이어도 collapse/해소 경로(자기 인지→정화→자기희생 등)가 반드시 존재. 막다른 운명 강제 금지.
  - 주체 플레이어가 무의식중 괴담 행동을 하게 되는 연출은 GM이 은밀히 처리(can_impersonate/perception P44·P60, 단일 주체 P24와 정합). 본인 입력을 강탈하기보다 '환경·결과'로 간접 암시.
  - 단, 관찰자/회피 전용 무력 배역 금지(GdamGenerator:117)는 그대로 유효 — 연루 배역도 능동적 해결 주체여야 함.
- [효과] '본인이 괴담임을 모른 채 해결' '씌인 자가 열쇠' 같은 반전 서사 가능. 엔딩 reveal(P61)에서 "정체 = 너 자신/네가 씌였다" 트위스트는 발견 시에만 공개.
- [코드 메모] 대부분 생성·hidden_info(기존 기제)로 처리 가능. 주체 플레이어의 무의식 행동 간접 연출이 코드 보조가 필요하면 CODE-19로 분리(저우선).
