...
*프롬프트 패치 P1~P70(+P44b) | 코드 TODO CODE-1~18(+CODE-19 저우선) (HIGH 10) | 그룹 G1~G26*

---

## G26. 괴담 연루 배역 허용 — 본인 미인지 (P70) — 사용자 교정 8 (#16 재정의)
**severity:** HIGH
**대상:** `GdamGenerator.java` 배역(roles)·hidden_info 설계 + `TRPGGameManager.java` GM_SYSTEM_BASE 반스포일러 운용
**핵심:** #16은 '금지'가 아니라 ★'허용 + 본인 미인지'.
**삽입 텍스트:**
```
★ 괴담 연루 배역(허용·은닉):
- 플레이어 배역이 ① 괴담에 씌인 피해자(→ 괴담에 큰 접근 권한을 가진 '키' 역할 가능), 또는 ② 괴담의 주체(발생원) 자체일 수 있다.
- 단 ★본인은 그 사실을 모르는 상태로 시작한다 — 연루 사실은 hidden_info에 두고, GM은 즉시 노출 금지·단서로 점진 공개(반스포일러 P41, 발견형 reveal P61).
- ②의 경우 플레이 중 단서로 깨닫고 '자기 자신'을 해결해야 한다(자기 인지→정화→자기희생 등).
- ★해결 경로 보장(P24/P67): 연루 배역이어도 collapse/해소 경로가 반드시 존재(막다른 운명 강제 금지).
- 주체 플레이어의 무의식 괴담 행동은 GM이 은밀히 '환경·결과'로 간접 암시(입력 강탈 금지, can_impersonate/perception P44·P60와 정합).
- 관찰자/회피 전용 무력 배역 금지(기존 규칙)는 유지 — 연루 배역도 능동적 해결 주체.
- 엔딩 reveal(P61) "정체 = 너 자신/네가 씌였다" 트위스트는 발견 시에만 공개.
```
**코드 메모:** 대부분 생성·hidden_info(기존)로 처리. 주체 플레이어 무의식 행동 간접 연출이 코드 보조 필요 시 CODE-19(저우선).
