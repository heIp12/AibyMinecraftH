...
- **결정타 저굴림 '질질 끌림' 방지 지도**(회귀 주의): "핵심행동 완료→종결 판정 저굴림→드래그" 는 ★프롬프트 규칙 위주★로 잡혀 있고 하드 엔진 게이트는 없다. (1) DICE·CLEAR 분리(PB:251) — 결정타는 <DICE>만 내고 멈춤, ★성공 시 다음 응답에서만★ <CLEAR>(저굴림 실패 시 그 턴 종결 안 함 = 의도된 정상). (2) PB:282 부분성공·실패는 이미 이룬 진행 삭제 금지, 부분=누적, ★2턴 내 결판★, 대가는 시간·소음·부상. (3) PB:424~427 결정적 단일행동 1회 판정, 실패 시 같은 행동 재굴림 금지→새 전개·대가, 같은 대치 비트 3턴+ 재탕 금지. (4) PB:255 조건 쌓일수록 dc↓, 조건 다 모였는데 고dc 계속 실패=정석이행 판정거부(금지), 위협0 근처 결정타 계속 빗나가면 dc 과도. (5) 엔진 threatDcBump(4748)=위협 낮으면 DC 가산 0(저위협에선 판정 안 어려워짐), SPR은 아는 정보 많을수록 effDc↓. (6) 최후 백스톱=제한시각 초과 워치독(~511-534) "[종국—반드시 매듭]" 주입+강제 종결(소프트락 차단, 단 시간 기반). ★남은 갭★: 결정타 dc는 GM이 정하고 엔진은 위협·스테이지로 올리기만(SPR정보 완화 제외) — "조건 충족·저위협인데 연속 저굴림"을 엔진이 직접 종결 강제하는 장치 없음(GM 준수 의존). 하드 보장 원하면 (연속 결정타 실패 N회 후 강제 종결 / effDc 바닥) 추가 필요.
