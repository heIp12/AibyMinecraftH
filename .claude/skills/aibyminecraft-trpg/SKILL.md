...
- **★생성 정합 리마인더(사용자) — ★반영 완료★**: 괴담↔컨셉 ★상호★ 정합(괴담에 맞춰 컨셉, 컨셉에 맞춰 괴담) + 마지막
  ...
  개인사 무작위 무방★. 구현: (Part1) GdamGenerator 설계순서 4번 '상호 정합(양방향)' — 괴담→스토리 일방 파생만 말고
  컨셉과도 맞물리게 되짚기(PROMPT_1A). (Part2) TRPGGameManager.castHintFor 피날레 복귀캐스트 힌트에 '이름·성별·나이·직업만
  고정, 사연·관계·동기는 배역·나이·성별에 맞춰 새로(이음 억지 금지, 무작위 무방)' 추가(런타임 코드 스트링). ★생성 품질
  실효는 실플레이 검증 필요(이 환경 미검증) — 컴파일·바이트만 확인.★ ('3스' 표기는 stage-count 불명확 → 스테이지수 무관
  원칙만 반영, 필요시 사용자 확인).
