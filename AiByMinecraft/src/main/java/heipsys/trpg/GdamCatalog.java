...
""";
    ...
    private static int fameWeight(Fame f, int stage, String famePool) {
        if (famePool != null) switch (famePool) {   // /trpg setting 인지도풀 고정(유명/덜유명/마이너만) — 강한 편향(잔여 5, 완전배제는 아님)
            case "major" -> { return f == Fame.MAJOR ? 100 : 5; }
            case "semi"  -> { return f == Fame.SEMI  ? 100 : 5; }
            case "minor" -> { return f == Fame.MINOR ? 100 : 5; }
            default -> { }   // "" / "stage" → 스테이지 곡선(난이도별)
        }
    ...
    public static List<Entry> pick(String src, int stage, Set<String> avoid, int count, String famePool) {
        ...
        while (!pool.isEmpty() && out.size() < count) {
            ...
            for (int i = 0; i < pool.size(); i++) {
                w[i] = (long) fameWeight(pool.get(i).fame(), stage, famePool) * scaleWeight(pool.get(i), stage);
                total += w[i];
            }
            if (total <= 0) break;
