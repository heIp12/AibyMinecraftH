...
public class SystemTraitRegistry {
    ...
    public enum Effect {
        ...
        GUARANTEED("guaranteed", true,
            "발동 시 ★이 특성이 정한 종류의 행동★ 1회를 확정 성공으로 처리한다(주사위·실패를 무시하고 GM이 성공으로 서술). "
            + "회피가 아니라 '반드시 성공'하는 결과 보장이다. ★effect에 어떤 종류의 행동인지 반드시 명시하라(예: '격투로 제압하는 행동', '자물쇠·기계를 여는 행동'). "
            + "취지와 무관한 다른 행동에는 적용되지 않는다(아무 행동이나 확정 성공이 아니다).",
        ...
        // ─── 신규 특이 능력 (Project Moon 톤 — 전부 GM이 세부·강도·결과를 조정할 여지를 둔다) ─────────────
        TRUTH_READ("truth_read", true,
            "발동 시 ★가장 최근 NPC의 발언★이 진실/거짓/괴담에 오염된 정보 중 무엇인지 그 인물의 태도·정황으로 은근히 드러낸다(색·확언 아님, 뉘앙스로). ★남용하면 그 NPC가 경계해 입을 닫는다(자기제한).",
            "uses=스테이지당 횟수(1~3)"),
        PITFALL_REVEAL("pitfall_reveal", true,
            "발동 시 이번 사건의 '★절대 하면 안 되는 것★' 1가지(가짜 규칙·함정 지시·치명 금기 중 하나)를 확정으로 폭로한다. 대가로 괴담이 발동자를 '규칙을 아는 자'로 우선 표적화한다.",
            "uses=스테이지당 횟수(1~2)"),
        MAD_CLARITY("mad_clarity", true,
            "발동 시 정신력을 크게 소모하고 그 순간 괴담의 규칙 조각 하나를 또렷이 본다. ★정신력이 낮을수록 통찰이 더 선명하다(위험할수록 잘 보임).",
            "cost=정신력 소모(3~15), uses=스테이지당 횟수(1~2)"),
        FEAR_TRANSFER("fear_transfer", true,
            "발동 후 대상을 정하면 자신의 공포를 그 대상에게 떠넘긴다 — 아군이면 위축(배신), 적/중립 NPC면 도주·붕괴, ★괴담이면 잠시 주춤하되 분노가 치솟는다(도발).",
            "uses=스테이지당 횟수(1~2)"),
        DEBT("debt", true,
            "발동 후 아군 1명을 지정하면 그가 '빚'을 진다 — 다음에 발동자가 위기일 때 ★1회 강제로 그를 돕게 된다★. 저버리면 그에게 불이익(낙인).",
            "uses=스테이지당 횟수(1~2)"),
        NAME_STEAL("name_steal", true,
            "발동 후 NPC 이름을 지정하면 잠시 그 사람 행세를 한다 — 다른 이들이 발동자를 그 사람으로 인식한다(사칭). ★발각되면 큰 대가·표적화.",
            "turns=지속 턴(1~3), uses=스테이지당 횟수(1)"),
        WITNESS_PACT("witness_pact", true,
            "발동 후 계약 내용을 입력하면 두 대상 사이에 강제 계약이 성립한다 — ★먼저 어긴 쪽이 괴담의 표적★이 된다(이간·협상·함정용).",
            "uses=스테이지당 횟수(1~2)"),
        FUTURE_SIGHT("future_sight", true,
            "발동 시 ★확정된 미래★ — 곧 반드시 닥칠 다음 사건(다가오는 전개)을 생생한 예지로 미리 본다. 만드는 예약이 아니라 정해진 미래를 '보는' 것. 정답·해결법은 제외.",
            "depth=예지 선명도(1=흐릿한 조짐, 2=장면, 3=또렷한 전조), uses=스테이지당 횟수(1~2)"),
        CAUSAL_DEBT("causal_debt", true,
            "발동 시 다음 행동 1회를 ★확정 대성공★으로 만든다 — 대신 그 값을 '미래의 나'에게 청구해 몇 턴 뒤 그만큼의 재앙이 자동으로 닥친다(즉효·후불).",
            "turns=재앙까지 지연(2~4), uses=스테이지당 횟수(1)"),
        RULE_INVERT("rule_invert", true,
            "발동 시 괴담 규칙 1개를 잠시 ★거꾸로 뒤집는다★(예: '보면 죽는다'→'봐야 산다'). 판을 엎지만 나머지 규칙이 더 사나워지고 분노가 치솟는다.",
            "turns=지속 턴(1~2), uses=스테이지당 횟수(1)"),
        FEED_ENTITY("feed_entity", true,
            "발동 후 무엇을 바칠지(단서·물건·기억) 정하면 괴담에게 먹여 잠시 온순하게 만든다 — 공격이 멎지만 ★괴담이 그걸 먹고 성장한다(양날).",
            "turns=진정 턴(1~2), uses=스테이지당 횟수(1~2)"),
        LAST_WORDS("last_words", true,
            "발동 후 유언을 입력하면 그 한마디가 ★확정 단서★로 가까운 아군(또는 다음 스테이지)에 전달된다. 죽기 직전에 쓰면 죽음을 정보로 바꾼다.",
            "uses=스테이지당 횟수(1)"),
        EMPTY_CHAIR("empty_chair", true,
            "발동 시 보이지 않는 '한 명'을 파티에 더한다 — 괴담이 인원을 착각해 ★표적을 분산★시킨다(허수 인원).",
            "turns=지속 턴(2~4), uses=스테이지당 횟수(1~2)"),
        VANISH("vanish", true,
            "발동 시 ★지정 대상의 인식에서 N턴간 사라진다★ — 그 대상은 발동자를 보거나 찾지 못한다. ★S급이면 괴담에게까지 적용★된다(그 외 등급은 사람·NPC 대상).",
            "turns=지속 턴(1~3), uses=스테이지당 횟수(1~2)"),
        ILLUSION("illusion", true,
            "발동 후 보여줄 환영/암시를 입력하면 대상에게 ★환영을 보여주거나 약하게 조종★한다(완전 지배 아님 — 가벼운 현혹·오인·충동). 괴담 본체엔 통하지 않는다.",
            "power=현혹 강도(1=가벼운 착시, 2=뚜렷한 환영·약한 조종), uses=스테이지당 횟수(1~2)"),
        ITEM_CREATE("item_create", true,
            "발동 시 도움이 되는 아이템 하나를 만들어 손에 넣는다 — 도구·무기·방어구 등 ★상황에 맞는 것을 GM이 자유롭게 정한다★. 원하는 종류를 함께 적을 수 있다.",
            "power=아이템 위력(1=소소한 도구, 2=쓸만한 장비, 3=강력한 병기), uses=스테이지당 횟수(1~2)"),
    ...
    private static final List<Preset> PRESETS = List.of(
        ...
        new Preset("sys_encounter_a", "조우자", "A", "우군 NPC 조우",
            "발동 시 진행을 돕는 ★실제 조력 NPC★가 등장한다(대화·연락 가능). 기존 우호 NPC가 없으면 새 조력자를 만들어 등장.",
            -1, "force_encounter", p()),
        new Preset("sys_encounter_s", "강령 조우자", "S", "강력 NPC 조우",
            "발동 시 강력한 ★실제 조력 NPC★(회귀자·재단원·영적 존재 등)가 등장해 돕는다(대화·연락 가능).",
            -1, "force_encounter", p()),
