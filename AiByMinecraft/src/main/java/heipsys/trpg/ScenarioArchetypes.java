...
""";
    ...
    public static String worldRulesBlock(int stage, String typeHint) { return worldRulesPick(stage, typeHint, java.util.Collections.emptySet()).prompt(); }

    /** 세계 규칙 블록 + 이번 회차 '주 구조'로 엔진이 권장한 표준 아키타입 id(중복방지 기록·폴백용). */
    public record WorldRules(String prompt, String primaryArchId) {}

    /** ★아키타입 표준 id★ — 텍스트 앞머리 이름(— 앞, 괄호 부제·공백 제거). 캠페인 중복방지 키.
     *  '정보 격리형 —…'→"정보격리형", '자동 해결형(비개입형) —…'→"자동해결형". */
    public static String archId(String text) {
        if (text == null) return "";
        String s = text;
        int dash = s.indexOf('—'); if (dash < 0) dash = s.indexOf('–');
        if (dash >= 0) s = s.substring(0, dash);
        return s.replaceAll("\\([^)]*\\)", "").replaceAll("\\s+", "").trim();
    }
    /** 모든 RULE 아키타입 표준 id (검증·정규화용, 등장 순서 유지). */
    public static java.util.List<String> ruleArchIds() {
        java.util.List<String> ids = new java.util.ArrayList<>();
        for (Arch a : RULES) if (a.slot() == Slot.RULE) ids.add(archId(a.text()));
        return ids;
    }
     ...
     * typeHint(운영 설정 유형 고정)가 있으면 그 유형을 ★최우선 강제★(중복방지보다 우선).
     * ★캠페인 중복방지(GPT)★: excludeIds에 이번 캠페인에서 쓴 주 아키타입 id를 넣으면 ★엔진 샘플링 단계에서★
     * 후보에서 제외한다(프롬프트 힌트가 아니라 실제 후보에서 뺌 — 저품질 모델 우회 방지). 소진 시 전체 재허용.
     */
    public static WorldRules worldRulesPick(int stage, String typeHint, java.util.Set<String> excludeIds) {
        ThreadLocalRandom r = ThreadLocalRandom.current();
        boolean basic = basicOnly(stage);
        java.util.Set<String> excl = (excludeIds == null) ? java.util.Collections.emptySet() : excludeIds;
        String hint = typeHint == null ? "" : typeHint.trim();
        boolean typeForced = !hint.isEmpty() && !hint.startsWith("성격:");
        Arch forced = typeForced ? findArchByHint(hint) : null;               // 사용자·운영 지정(중복방지 예외)
        String forcedId = forced != null ? archId(forced.text()) : null;

        // 슬롯/티어 적격 풀
        List<Arch> abAll = new ArrayList<>(), cAll = new ArrayList<>(), openers = new ArrayList<>();
        for (Arch a : RULES) {
            if (basic && (a.tier() != Tier.A || a.slot() != Slot.RULE)) continue; // 초반: 어려운 효과·오프너 제외
            if (a.slot() == Slot.OPENER) openers.add(a);
            else if (a.tier() == Tier.C) cAll.add(a);
            else abAll.add(a);
        }
        // ★캠페인 중복 제외(엔진 샘플링)★ — forced(지정 유형)는 제외하지 않음. 소진(<5)이면 전체 재허용(최고참부터 근사).
        List<Arch> ab = new ArrayList<>();
        for (Arch a : abAll) { String aid = archId(a.text()); if (excl.contains(aid) && !aid.equals(forcedId)) continue; ab.add(a); }
        if (ab.size() < 5) ab = new ArrayList<>(abAll);
        List<Arch> c = new ArrayList<>();
        for (Arch a : cAll) if (!excl.contains(archId(a.text()))) c.add(a);
        ...
        if (forced != null) {
            pick.remove(forced); pick.add(0, forced); // 매칭 아키타입을 맨 앞으로
            String block = format("★유형 고정(운영 설정 — 최우선): 이번 회차 world_rules는 반드시 ★" + hint
                + "★ 유형으로 설계하라 — 아래 후보 중 이 유형에 맞는 것을 고르거나 이 유형으로 직접 창작하고, 다른 유형으로 새지 마라", pick);
            return new WorldRules(block + archetypeEcho(forcedId), forcedId);
        }

        String header;
        if (basic) {
            header = "세계관 규칙(world_rules) 후보 — 이 중 ★성격이 다른 것을 고르거나★ 같은 결의 새 유형을 창작하라(직전 방과 다른 유형으로) · 초반 스테이지라 ★기본 규칙★ 위주(복잡한 변칙은 다음 스테이지부터)";
        } else {
            // 후반(3+): 구조 우선 — ★첫 항목을 고정 구조★로, 괴담(entity.type 포함)을 여기 맞춰 도출.
            header = "★구조 우선(후반) — world_rules 구조를 ★먼저 고정★: 반드시 ★아래 첫 번째 구조★로 world_rules를 설계하고, 괴담의 정체·★entity.type★·규칙·타임라인을 ★이 구조에 맞춰 도출★하라(entity.type을 먼저 정하지 마라 — 구조가 먼저). 나머지는 변형 참고(첫 구조가 정 안 맞을 때만 택1, 직전 방과 다른 유형으로)";
        }
        String primaryId = pick.isEmpty() ? "" : archId(pick.get(0).text()); // 후반=첫 항목 고정 / 초반=권장 주 구조(폴백용)
        ...
        return new WorldRules(block + archetypeEcho(primaryId), primaryId);
    }

    /** world_rules.archetype 필드 출력 지시 — 모델이 표준 id로 주 구조를 신고하게(자유 이름 창작 금지, 중복방지 기록용). */
    private static String archetypeEcho(String primaryId) {
        return "\n★world_rules.archetype 필드★: 이번 판의 ★주 구조★ 표준 id를 정확히 적어라"
            + (primaryId == null || primaryId.isEmpty() ? "" : "(권장: \"" + primaryId + "\")")
            + " — ★위 후보에 쓰인 표준 id 그대로★(정보격리형·시간·인과율형·역할극강제형 등), 새 이름을 지어내지 마라(자유 이름은 중복방지를 우회한다).\n";
    }
