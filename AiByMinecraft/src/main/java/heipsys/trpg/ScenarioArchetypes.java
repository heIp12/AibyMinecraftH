...
""";
    ...
    public static String rolesBlock(int stage) {
        ...
        return format("NPC 역할(role_type) 후보 — 한 방에 ★서로 다른 역할★을 섞어라(전부 적대/전부 조력 금지). 진짜 역할은 처음부터 드러내지 마라. ★한 NPC에 역할 2개 이상 조합 가능 — role_type에 둘 다 표기하라(예: '방어막&쾌락주의자' / '방어막+열쇠'=살려야 하고 정보도 얻어야 하는 딜레마)", pick);
    }
