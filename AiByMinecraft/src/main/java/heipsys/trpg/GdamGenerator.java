...
""";
    ...
    private void finalizeNpcSpeech(JsonObject gdam) {
        try {
            ...
            // ★개성 어미 회전 풀(14종)★ — 시나리오마다 nextRotatingEnding으로 순환 배정해 '늘 같은 어미(특히 ~라니까)'를 없앤다.
            //   ★모델이 넣은 어미도 이 풀로 덮어쓴다★(모델이 프롬프트 예시 '~라니까'를 verbatim 복제하는 편향의 근본 해소).
            //   존/반 방향은 게임이 상대·관계로 정하고 그 위에 어미만 얹힌다 — 존대 예시가 붙은 항목(격식체 등)은 대체로 존대에 얹힌다.
            String[] pool = {
                ...
                "격식 차려 '~입니다/~올시다'로 딱딱하게 맺는다 (예: '그래'→'그렇습니다', '아니야'→'아니올시다') — 대체로 존대에 얹힌다",
