...
★ 단, 구조를 획일화하지 마라. 괴담마다 형태가 완전히 달라야 한다:
  - 규칙형: 규칙이 별도 주체이고 entity·NPC는 그 규칙을 구현·집행하는 수단.
  - 단일 존재형: 쿠네쿠네처럼 '그 존재 자체가 곧 규칙'인 경우. 이때 core는 그 존재의 본질을 적는다.
  - 정보 격리형: ★특정 1명(또는 일부)만 어떤 사실을 '알면' 표적이 되는★ 규칙(아는 자가 위험, 모르는 자가 안전).
    이런 방은 그 사실을 ★최후반에야★ "○○만은 이것을 알면 안 된다"고 드러내고, 다른 배역에겐 ★아주 간접적으로만★ 흘려
    스스로 '저 사람에겐 숨겨야 한다'를 눈치채게 설계하라(직접 명시 금지 — 그래야 긴장·재미가 산다).
    ※ 이미 아는 자(다회차·메타지식 보유자)는 시작부터 표적이 되어 오히려 더 위험하다.
  - 통신 유인형: ★전화·무전·@통신을 '사용하는 행위 자체'가★ 괴담을 부르거나 응답하게 하는 규칙(통신은 작동하되 쓰면 위험).
    이때 constraints.comms_dangerous=true로 표시하고, world_rules에 '통신 시 무엇이 응답·추적하는가'를 명시하라.
  - 자동 해결형(비개입형): ★괴담이 스스로, 또는 특정 NPC가★ 시간이 지나면 사건을 매듭짓는다(예: NPC가 의식을 끝까지 마치면 봉인, 괴이가 제풀에 떠남). 플레이어가 ★개입하면 오히려 악화·실패★하고, 방해하지 않고 두면 해결된다 — 결국 '가만히 있었으면 클리어'다.
    ★함정의 핵심: 그 NPC·괴이의 행동이 플레이어 눈엔 '위험·해악'처럼 보여야 한다(예: 누군가를 해치는 듯한 의식, 무언가를/누군가를 묻는 듯한 행동). 그래야 말리고 싶어지고 — 말리면(개입하면) 진행이 깨져 악화한다.
    표시: constraints.noninterference=true. world_rules.details에 ①무엇이 스스로 진행되어 해결로 가는가 ②어떤 개입이 그것을 망치는가를 명시. collapse_condition은 '핵심 진행을 방해하지 않고 끝까지 두는 것(비개입)'으로 둔다. loophole=단서로 '그 행동이 사실은 해결 과정'임을 알아채면 개입 충동을 참을 수 있음.
    ★단조로움 방지: '아무것도 안 하면 무조건 클리어'가 되지 않게 — 괴이·정황이 플레이어를 자극·위협해 개입을 유도하고, 단서로 진상을 파악해야 '안심하고 비개입'할 수 있게 설계하라(진실을 모르면 참기 어렵게).
  매번 같은 틀(예: 항상 별도 규칙+집행 NPC)로 찍어내지 마라. 위 유형들을 적극 활용해 매번 다르게 설계하라.
...
- phone_usable: 통신기기(전화·무전 등)가 작동하는가.
  * 신호 교란·전파 차단·시대상 부재면 false. 영역형은 보통 외부 통신이 끊긴다.
...
- 발생원: 괴담을 일으키거나 유지. 제거하면 괴담 소멸 가능. (반복되면 공식이 되니 남발 금지)
- 방어막: 오히려 괴담을 억제 중. 제거하면 괴담이 폭주·강화된다(반전). 사전에 알 수 없어야 함.
- 제물: 제거하면 오히려 괴담 조건이 충족됨. 살려야 막을 수 있는 역설.
- 열쇠: 해결에 필요한 정보/아이템을 유일하게 보유. 죽이면 퍼펙트 경로가 막힘(죽이기 전 정보 확보 필수).
- 피해자: 이미 잠식된 존재. 제거해도 본질은 남음(일반 클리어만 가능).
- 무관: 괴담과 무관. 불필요한 제거는 평가 하락만.
- 복합: 위 둘 이상 동시(예: 방어막+열쇠 — 살려야 하고 정보도 얻어야 하는 딜레마).
규칙:
...
""";
    ...
    private static final int FAMILIAR_HISTORY_MAX = 30;
    ...
    private CompletableFuture<String> generateFamiliarConcept(int roomNumber, String filter) {
        ...
        return aiManager.callAssistant(task, data).thenApply(raw -> {
            ...
            return wrapFamiliarConcept(lore, roomNumber);
    ...
    private String wrapFamiliarConcept(String lore, int roomNumber) {
    ...
    public List<String> listSavedSeeds() {
        java.util.TreeSet<String> seeds = new java.util.TreeSet<>();
