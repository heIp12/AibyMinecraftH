...
""";
    ...
    /** 이번 플레이에서 이미 사용한 친숙 괴담 이름 — 중복 방지 (스테이지 1에서 초기화) */
    private final java.util.Set<String> usedFamiliar = java.util.concurrent.ConcurrentHashMap.newKeySet();
    ...
    private CompletableFuture<String> generateFamiliarConcept(int roomNumber) {
        if (roomNumber <= 1) usedFamiliar.clear(); // 새 플레이 시작 — 중복 추적 초기화
        ...
        String avoid = usedFamiliar.isEmpty() ? ""
            : "이번 플레이에서 이미 쓴 괴담(중복 금지): " + String.join(", ", usedFamiliar) + "\n";
        ...
        return aiManager.callAssistant(task, data).thenApply(raw -> {
            ...
            for (String line : lore.split("\n")) {
                ...
                if (t.startsWith("이름:")) {
                    ...
                    if (!nm.isEmpty()) usedFamiliar.add(nm);
                    break;
                }
            }
    ...
    private String lengthGuideFor(int room) {
        ...
        String target = switch (scale) {
            case "로컬"   -> "daily_turns≈3 · 괴담 약 6~9턴 · main_events 3개 · zones 2~3개 · clues 3~5개";
            case "시티"   -> "daily_turns≈4 · 괴담 약 10~14턴 · main_events 4개 · zones 3~4개 · clues 5~7개";
            case "내셔널" -> "daily_turns≈5 · 괴담 약 14~18턴 · main_events 5개 · zones 4~5개 · clues 7~9개";
            case "글로벌" -> "daily_turns≈6 · 괴담 약 18~24턴 · main_events 5~6개 · zones 5~6개 · clues 9~11개";
            default        -> "daily_turns≈6 · 괴담 약 24~30턴 · main_events 6개 · zones 5~6개 · clues 10~12개"; // 코즈믹/복합
        };
        return "## 챕터 분량 (스케일=" + scale + "에 따라 자동 결정 — 반드시 준수) ★\n"
            + "- 목표 분량: " + target + ".\n"
            + "- 괴담 파트 길이는 start_time/end_time/minutes_per_turn으로 맞춘다: (end_time−start_time)/minutes_per_turn ≈ 위 '괴담 턴' 수가 되게 잡아라.\n"
            + "- 작은 스케일은 짧고 응축되게, 큰 스케일은 길고 탐색·이동이 풍부하게. 분량을 임의로 늘리거나 줄이지 마라.\n";
    }
