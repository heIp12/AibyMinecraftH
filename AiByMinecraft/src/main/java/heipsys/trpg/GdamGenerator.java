...
""";
    ...
    private void finalizeNpcSpeech(JsonObject gdam) {
        try {
            ...
            // ★개성 어미 회전 풀 — 시대(era) 정합 2계열★(외부 감사 P2: 현대 청년에 '~구먼요', 과거 인물에 '~다에요'가
            //   무작위로 떨어지던 문제) — 과거 배경엔 예스러운 계열만, 현대·미래엔 현대 구어 계열만 순환 배정한다.
            //   ★모델이 넣은 어미도 이 풀로 덮어쓴다★(프롬프트 예시 '~라니까' verbatim 복제 편향의 근본 해소).
            //   존/반 방향은 게임이 상대·관계로 정하고 그 위에 어미만 얹힌다.
            String[] modernPool = {
            ...
            String[] pool = isPastEra(gdam) ? archaicPool : modernPool;
