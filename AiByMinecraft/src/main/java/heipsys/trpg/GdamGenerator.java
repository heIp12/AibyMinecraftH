...
## 출력 JSON 스키마
{
  ...
  "zones": [{"zone_id":"zone_A","name":"","accessible_by":[],"exclusive":false,"connections":["zone_B"]}],
  "npcs": [
...
""";
    ...
    public CompletableFuture<JsonObject> generate(int roomNumber) {
        // 1단계: 전 세계 괴담·전설에서 자유롭게 컨셉을 선택하거나 새로 발명한다.
        // 2단계: 그 컨셉에 맞춰 전체 시나리오를 설계한다(분할 또는 단일).
        return generateEntityConcept()
            .exceptionally(ex -> "")
            .thenCompose(concept -> {
                String c = sanitizeConcept(concept);
                return SPLIT_GENERATION ? generateChunked(roomNumber, c) : generate(roomNumber, 0, c);
            });
    }
     ...
     */
    private CompletableFuture<JsonObject> generateChunked(int roomNumber, String concept) {
        ...
        return aiManager.callGmAiLarge(GDAM_SYSTEM_PROMPT, corePrompt).thenCompose(coreRaw -> {
            ...
            if (core == null || !core.has("entity") || !core.has("timeline")) {
                ...
                return generate(roomNumber, 0, concept);
            }
            ...
            return aiManager.callGmAiLarge(GDAM_SYSTEM_PROMPT, rolesPrompt).thenCompose(rolesRaw -> {
                ...
                return aiManager.callGmAiLarge(GDAM_SYSTEM_PROMPT, itemsPrompt).thenCompose(itemsRaw -> {
                    ...
                    core.add("key_items", items != null ? items : new JsonArray()); // 아이템 실패는 비치명: 빈 배열 허용
            ...
            .thenCompose(raw -> {
                ...
                try {
                    ...
                    if (attempt < 1) {
                        ...
                        return generate(roomNumber, attempt + 1, concept);
                    }
