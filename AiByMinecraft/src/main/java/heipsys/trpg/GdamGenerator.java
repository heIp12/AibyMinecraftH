...
방마다 '고유한 세계관 규칙'이 존재하며, 이 규칙이 방 전체를 지배하고 플레이어를 압박하는 실질적 주체다.
괴담 존재(entity)와 NPC는 이 규칙을 '구현·집행'하는 수단일 뿐이다. 규칙이 먼저고, 위협은 그 표현이다.
- core: 이 방을 지배하는 핵심 규칙 한 줄. NPC가 전부 사라져도 규칙은 성립한다. (예: "해가 지면 이름이 불린 자는 사라진다")
- details: 규칙의 구체 조항 2~4개. '무엇을 어기면 무슨 일이 일어나는가'를 적는다. (예: "거울을 보면 표식이 생긴다")
- hidden: 플레이어가 처음부터 규칙을 아는가. 보통 true(모름) — 탐색·단서·희생으로 점차 파악해야 한다.
- loophole: 규칙을 역이용할 수 있는 '허점'. 반드시 존재해야 한다. 규칙을 모르면 죽고, 알면 역이용한다.
- collapse_condition: 이 조건이 충족되면 규칙이 성립 불가능해져 '괴담이 소멸'한다 = 해결(퍼펙트) 클리어 조건.
  ★ entity.weakness/solution/exploit_path는 바로 이 collapse_condition을 달성하는 '구체적 수단'이어야 한다(둘이 따로 놀면 안 됨).
- npc_dependency: 규칙이 NPC에 얼마나 의존하는가.
  * "low"(낮은 방): NPC를 제거하면 규칙이 수동화되어 종료 가능. 단 편법 — 평가 낮음. 규칙 자체를 깨면 높은 평가.
  * "mid": NPC를 제거해도 규칙 일부가 잔존. NPC 제거만으론 부족하다.
  * "high"(높은 방): 규칙이 NPC와 독립적으로 자동 진행. NPC를 없애도 괴담이 스스로 진행 — 규칙 자체/근본 원인을 깨야 한다.
- 금지: 세계관 규칙 없이 NPC만으로 구성된 방(NPC를 없애면 아무것도 없는 구조). collapse_condition이 없는 무한 위협. 회피만으로 끝나는 구조.
