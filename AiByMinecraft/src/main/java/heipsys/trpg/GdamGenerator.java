...
규칙 역이용은 반드시 아래 조건을 모두 충족해야 한다:
- 괴담의 실제 규칙에서 직접 파생되는 논리적 허점이어야 한다
- 플레이어가 규칙을 깊이 이해했을 때 자연스럽게 떠올릴 수 있는 방법이어야 한다
- 억지 해석이나 규칙 문자에만 의존하는 트릭은 금지 (예: "줄을 11개만 끊으면 멈춘다" — 실제 멈추는 이유가 없음)
- 역이용해도 안전하지 않아야 한다 (리스크·대가가 있어야 몰입감 유지)
- 실패 시 더 위험한 상황이 될 수 있어야 한다
- 반드시 1가지만 작성 (다수 역이용은 난이도를 지나치게 낮춤)
...
""";
    ...
    public CompletableFuture<JsonObject> generate(int roomNumber) {
        // 1단계: 먼저 독창적인 괴담 '존재' 컨셉부터 정한다.
        // 2단계: 그 컨셉에 맞춰 전체 시나리오를 설계한다.
        String themeHint = pickTheme();
        String[] insp    = pickInspiration();
        return generateEntityConcept(themeHint, insp)
            .exceptionally(ex -> "")  // 컨셉 생성 실패 시 컨셉 없이 진행
            .thenCompose(concept -> generate(roomNumber, 0, themeHint, sanitizeConcept(concept)));
    }
    ...
    // 무작위 영감 재료 풀 — 매 생성마다 의외의 조합으로 수렴(비슷한 괴담 반복)을 막는다.
    private static final String[] SETTING_POOL = {
        "폐병원", "심해 연구소", "시골 폐교", "막차 지하철", "산속 기도원", "재개발 아파트",
        "외딴 등대", "심야 방송국", "요양원", "지하 벙커", "폐점 직전 백화점", "고속도로 휴게소",
        "오래된 목욕탕", "야간 물류창고", "케이블카 정류장", "수몰 예정 마을", "심야 PC방",
        "장례식장", "폐쇄된 놀이공원", "고시원", "심야 편의점", "터널 공사현장", "온천 료칸"
    };
    private static final String[] MECHANIC_POOL = {
        "특정 규칙을 어기면 즉시 대가를 치른다", "정해진 시각에만 활성화된다",
        "쳐다보거나 인지하는 순간 표적이 된다", "이름을 부르면 가까워진다",
        "소리를 내면 위치가 발각된다", "숫자를 세거나 순서를 어기면 작동한다",
        "행동을 거꾸로 해야 안전하다", "잠들거나 눈을 감으면 잠식한다",
        "기록하거나 말하면 현실이 된다", "같은 행동을 반복하면 루프에 갇힌다",
        "특정 물건을 지니면 노출된다", "감정(공포·분노)에 반응해 강해진다"
    };
    private static final String[] MOTIF_POOL = {
        "맡으면 안 되는 냄새", "반복되는 한 구절", "점점 늘어나는 무언가",
        "사라지는 사람들", "멈추지 않는 소리", "비정상적인 온도 변화",
        "잘못된 그림자", "바뀌어 있는 기록", "되돌아오는 물건", "틀린 숫자·시간",
        "낯선 흔적", "닫히지 않는 문"
    };
    ...
    private String[] pickInspiration() {
        var rnd = java.util.concurrent.ThreadLocalRandom.current();
        return new String[]{
            SETTING_POOL[rnd.nextInt(SETTING_POOL.length)],
            MECHANIC_POOL[rnd.nextInt(MECHANIC_POOL.length)],
            MOTIF_POOL[rnd.nextInt(MOTIF_POOL.length)]
        };
    }

    /** 1단계: 무작위 영감을 의외의 방식으로 조합해 독창적 괴담 컨셉을 먼저 만든다. */
    private CompletableFuture<String> generateEntityConcept(String themeHint, String[] insp) {
        String task = "너는 호러 괴담 컨셉 디자이너야. 아래 무작위 영감 재료를 의외의 방식으로 조합해 "
            + "지금까지 본 적 없는 '독창적인 괴담 존재' 하나를 구상해라.";
        String data = "소재 방향: " + themeHint + "\n"
            + "영감 재료(그대로 베끼지 말고 영감으로만 활용): 배경=" + insp[0]
            + " / 기믹=" + insp[1] + " / 모티프=" + insp[2] + "\n"
            + "제약: 거울·반사·도플갱어·단순 백색 귀신 등 클리셰 금지. 흔한 전개 회피.\n"
            + "출력(평문 4~6문장, JSON 금지): "
            + "1) 괴담의 정체와 이름 2) 핵심 작동 규칙(기믹) "
            + "3) 약점·파훼 조건 4) 숨은 반전 한 가지. 구체적이고 간결하게.";
    ...
    private CompletableFuture<JsonObject> generate(int roomNumber, int attempt,
                                                   String themeHint, String concept) {
            ...
            .thenCompose(raw -> {
                ...
                try {
                    ...
                    if (attempt < 1) {
                        ...
                        return generate(roomNumber, attempt + 1, themeHint, concept);
                    }
