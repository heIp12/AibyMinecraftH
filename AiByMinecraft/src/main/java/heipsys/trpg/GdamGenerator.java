...
public class GdamGenerator {
    ...
    // ──────────────────────────────────────────────────────────────
    private static final String GDAM_SYSTEM_PROMPT_1A = """
...
- ★친숙 IP 핵심 메커니즘 충실도(이름뿐 아니라 '플레이의 뼈대'도 원작을 따른다)★: 실존 IP(게임·전설) 기반이면 그
  괴담의 ★핵심 위협 방식·핵심 해결 루프★를 원작에서 도출하라 — 원작에 ★없는★ 일반 발명품(지하실 봉인 장치·톱니
  기계 제어반·구조 왜곡 코어·재부팅 스위치 류)을 해결 수단으로 지어내 원작을 덮지 마라(실측: '아오오니' 저택인데
  해결이 '지하실 톱니 봉인 장치 파괴'로 변질돼 저택이 기계 퍼즐이 됨).
  · ★추격·탈출형 IP(아오오니·팔로알토·추격귀 류)★: 해결은 원작대로 ★'그 괴물을 피해 저택/공간을 빠져나가는 것'★이다
    — collapse_condition을 억지로 '장치 파괴'로 채우지 말고 ★순수 회피/탈출형(collapse 없음, 스테이지 1~2)★으로
    잡거나, 원작에 실제 있는 요소(열쇠·숨기·원작 퍼즐)로만 escape를 구성하라. 없는 봉인 기구를 발명하지 마라.
  · weakness/solution/exploit_path/rules도 ★원작의 실제 약점·규칙★을 우선한다(빨간마스크=포마드·삼(3)·사탕 류). 원작에
    근거가 없을 때만 개연적으로 창작하되, 그때도 그 IP의 ★분위기·행동 양식★을 벗어나는 SF·기계 장치로 튀지 마라.
...
※ ★주도 방향은 스테이지가 정한다(아래 지시 우선)★: 1~2스테이지는 위처럼 괴담→구조(괴담에 맞춰 구조), 3스테이지+는 별도 '구조 우선' 지시대로 구조→괴담(고정 구조에 맞춰 괴담을 도출). 어느 쪽이든 ★주도자에 종속자를 확실히 맞춰 겉돌지 않게★ 하라.
...
""";
    ...
    // ★기계적 분할(1B/2)★ — 단일 String 리터럴 65535 UTF-8 한계 회피용. 내용 불변, String.join으로 원복(바이트 동일 검증됨).
...
### ★보정 크기 규율 (요청) — '개성'과 '난이도 보정'을 구분하라★
- ★기본(개성 보정)은 소폭★: 직업·성격이 드러나는 정도로만. 한 스탯당 대개 ±1~3, ★한 배역 총량도 작게★.
  hp_max_add/san_max_add는 특히 조심 — 기본 체력·정신력 풀이 작아 큰 값이면 캐릭터가 망가진다(±2 이내 권장).
  ★hp_max_add:-10·san_max_add:+20 같은 극단 스윙 절대 금지★(엔진이 4스탯 [-4,+6]·체/정신 [-3,+6]으로 잘라내 무효가 된다).
- ★큰 폭(대부분 스탯 +3~+5)의 상향은 '어려운 판'일 때만★: 괴담의 위력이 스테이지 기준보다 확실히 세서(플레이어 기본 스펙으로는 벅참)
  ★난이도 보정이 필요한 경우에만★ 부여한다. 예) 너프되지 않은 최상위 환상체(백야類)가 1~2스테이지에 등장 → ★진행 가능하도록
  모든 배역의 대부분 스탯을 +3~+5(최대 +6)만큼 올려 생존력을 맞춘다(전 배역 공통 상향 + 각자의 개성 보정은 그대로).
- ★난이도가 스테이지에 걸맞으면 상향하지 마라★: 무난한 판에서 스탯을 부풀리면 성장이 난이도를 앞질러 시시해진다.
  즉 role_stats의 큰 양수 보정은 '괴담 위력 ↔ 스테이지'의 불균형을 메우는 용도이지, 매 판 기본으로 주는 보너스가 아니다.
...
""";
    ...
    private static final String GDAM_SYSTEM_PROMPT_2A = """
...
""";
    ...
    // ★기계적 분할(2B/2)★ — 65535 UTF-8 리터럴 한계 회피. 내용 불변(String.join으로 원복).
    private static final String GDAM_SYSTEM_PROMPT_2B = """
## 출력 JSON 스키마
{
  "seed": "",
  "room": 0,
...
""";
    ...
    static String[] rotatingEndingPool(boolean pastEra) {
        // ★어미 묶음(미니 문법) 형식★ — 각 항목이 '어떤 문장(기능)에 · 존/반 어느 형을 · 어떻게(교체) 쓰는지'와 변환 예시를
        //   스스로 선언한다. 적용기(restyleDialogue)는 특정 어미를 몰라도 이 선언·예시만 따르면 된다(스타일 불가지론).
        //   예전 단일 어미+예시 형식은 의문 성격 어미(~냐)가 평서문에 덧붙어 '바보군냐' 같은 비문을 낳았다(실사고).
        String[] modernPool = {
            "차분히 단정하는 군요체 — 평서·평가·깨달음에만: 존대 '~군요', 반말 '~군/~는군' (예: '멍청해'→'멍청하군요', '사람이 아니야'→'사람이 아니로군'). 의문·명령·감탄은 원형 유지",
            "따지듯 몰아붙이는 냐체(반말) — 질문·반문: '~냐' ('살아있어?'→'살아있냐'), 평서를 되받아 따질 때: '~냐고' ('왜 그래'→'왜 그러냐고'), 존대로 말할 땐 '~냐고요'만 가끔. 담담한 사실 전달·명령·감탄은 원형 유지. 기존 어미를 ★교체★하라('그렇군냐'식 겹침 금지 → '그렇냐')",
            "느릿하게 끝을 늘이는 지체 — 평서·수긍: '~지…' ('알았어'→'알았지…', '그래'→'그렇지…'), 확인 질문에도 가끔: '~지…?' ('갔어?'→'갔지…?'). 명령·감탄은 원형",
            "새침하게 받아치는 거든체 — 해명·반박 평서에: 반말 '~거든', 존대 '~거든요' ('바빠'→'바쁘거든', '아니야'→'아니거든요'). 담담한 사실 전달 절반은 평범한 어미로, 질문·명령은 원형",
            "되받아치는 잖아체 — 상기·반박 평서에: 반말 '~잖아', 존대 '~잖아요' ('몰라'→'모르잖아', '맞아'→'맞잖아요'). 처음 알려주는 설명은 평범한 어미로, 질문은 원형",
            "답답한 듯 강조하는 단말이야체 — 호소·강조 평서에만: 반말 '~단 말이야', 존대 '~단 말이에요' ('몰라'→'모른단 말이야'). 한 응답에 1~2번만, 나머지는 평범한 어미. 질문·명령 원형",
            "다그치는 라니까체 — 재촉 평서·명령·청유에: '~라니까/~자니까/~하라니까' ('가자'→'가자니까', '몰라'→'모른다니까', '해'→'하라니까'). 차분한 문장·질문은 원형",
            "슬쩍 흘리는 는데체 — 평서·완곡한 반응에: 반말 '~는데', 존대 '~는데요' ('봤어'→'봤는데', '괜찮아'→'괜찮은데요'). 질문·명령 원형",
            "'~다에요'체로 귀엽게 맺는다(이 인물만의 비표준 말투 — 의도된 형태다) — 평서·단정: '~다에요/~는 거에요' ('알았어'→'알았다에요', '그래'→'그런 거에요'), 질문·반문: '~냐에요?' ('그것도 몰라?'→'그것도 모르냐에요?'). 짧은 감탄은 원형",
            "격식체 고정(상대 불문 — 캐릭터성, 반말 상대에게도 존대) — 평서: '~입니다' ('그래'→'그렇습니다'), 질문: '~습니까' ('왜?'→'왜입니까'), 명령: '~하십시오'. 감탄·파편은 원형",
            "능청스레 이야기하듯 — 평서: 반말 '~단다', 존대 '~답니다' ('그래'→'그렇단다', '위험해'→'위험하답니다'), 권유: 반말 '~려무나'. 질문은 원형",
            "회상하듯 흘리는 더라체 — 직접 겪은 일 평서에만: 반말 '~더라', 존대 '~더라고요' ('있었어'→'있더라', '봤어'→'봤더라고요'). 지금 눈앞의 사실·질문·명령은 원형"
        };
        String[] archaicPool = {
            "차분히 단정하는 군요체 — 평서·평가·깨달음에만: 존대 '~군요', 반말 '~군/~는군' (예: '멍청해'→'멍청하군요', '사람이 아니야'→'사람이 아니로군'). 의문·명령·감탄은 원형 유지",
            "느릿하게 끝을 늘이는 지체 — 평서·수긍: '~지…' ('알았어'→'알았지…', '그래'→'그렇지…'), 확인 질문에도 가끔: '~지…?'. 명령·감탄은 원형",
            "하대하는 옛 물음체(반말) — 질문·반문: '~느냐/~더냐' ('살아있어?'→'살아있느냐', '봤어?'→'보았더냐'), 따져 물을 때: '~단 말이냐'. 담담한 평서는 예스러운 반말 그대로, 명령은 원형. 기존 어미를 ★교체★하라(겹침 금지)",
            "예스러운 격식체 고정(존대 — 캐릭터성) — 평서: '~올시다/~입니다' ('아니야'→'아니올시다'), 질문: '~니까'. 감탄·파편은 원형",
            "능청스레 이야기하듯 — 평서: 반말 '~단다', 존대 '~답니다' ('그래'→'그렇단다', '위험해'→'위험하답니다'), 권유: 반말 '~려무나'. 질문은 원형",
            "구성진 구먼체 — 평서·감탄에: 반말 '~구먼', 존대 '~구먼요' ('왔네'→'왔구먼', '좋아'→'좋구먼요'). 질문·명령 원형",
            "회상하듯 흘리는 더라체 — 직접 겪은 일 평서에만: 반말 '~더라', 존대 '~더라고요' ('있었어'→'있더라', '봤어'→'봤더라고요'). 지금 눈앞의 사실·질문은 원형",
            "짐짓 엄포 놓는 렷다체 — 명령·경고에만: '~렷다(!)' ('봐라'→'봐렷다', '조심해'→'조심하렷다'). 평서·질문은 원형"
        };
