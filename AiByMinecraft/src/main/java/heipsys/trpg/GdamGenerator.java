...
""";
    ...
    private CompletableFuture<String> generateFamiliarConcept(int roomNumber, String filter) {
        ...
        switch (filter == null ? "random" : filter) {
            ...
            case "projectmoon" -> { scope = "프로젝트 문(로보토미 코퍼레이션·라이브러리 오브 루이나)의 환상체(Abnormality)";
                criterion = "★실존 환상체만 — 새로 지어내지 마라. 아래 실제 환상체 목록에서 고르거나, 네가 확실히 아는 다른 실존 환상체만 택하라(불확실하면 목록에서):\n"
                    + "한 죄악과 수백의 선행 / 하얀 밤 / 괴물 같은 건 없어 / 그을린 소녀 / 백설공주의 사과 / 붉은 구두 / 증오의 여왕 / 절망의 기사 / 죽은 나비들의 장례식 / 침묵의 관현악단 / 벌하는 새 / 심판하는 새 / 큰 새(종말조) / 여왕벌 / 지옥행 급행열차 / 노래하는 기계 / 버려진 살인마 / 행복한 테디베어 / 마탄의 사수 / 마법소녀.\n"
                    + "★이름은 ★정식 한국어 명칭★ 그대로 — 프로젝트 문은 한국 제작사 IP다. 영어명(WhiteNight 등)을 임의로 재번역하지 말고 공식 한국어명(예: 하얀 밤)을 써라.\n"
        ...
        return aiManager.callAssistantHiFi(task, data).thenApply(raw -> { // 정전 충실도(창작 방지) — 고품질 모델로 선정
