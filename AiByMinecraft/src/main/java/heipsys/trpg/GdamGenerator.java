...
- 사육·이용형: 죽이지 않고 ★가둬 길러 수확★(공포·기력·노동)한다. ★생존 ≠ 안전★ — 사육장을 깨고 나가야 한다.
- 비탄·전염형(슬픔): ★절망·비탄을 전염★시킨다 — 곁에 두면 무력감·자해 충동. 희망·유대를 지켜 동화되지 않기.
- 정주형(원래 있던 것): 특정 ★구역에 묶여 늘 거기 있던★ 존재. 찾아오지 않고 그 구역에 들어온 자에게만 반응 — ★그 구역을 피하면 안전★(단 단서·풀이가 거기 있으면 들어가야 하는 딜레마).
...
- 환경동화형: ★우리 세계의 환경에 잠복★(벽·그림자·물·군중) — 어디 있는지 모름.
...
- 세계창조형: 괴담 ★고유의 물리 세계로 끌어들인다★(저택·역·엘리베이터 너머) — 그 안에선 괴담이 토착민, 우리가 이방인.
- 꿈·무의식형: ★수면·환각 상태에서만★ 괴담의 영역에 진입(깨면 안전하나 잠을 안 잘 순 없다).
★ 정합: 규칙(world_rules)·행동양식(type)·성향(disposition)이 ★자연스럽게 맞물리게★ 하라(억지 조합 금지 — 예: 직접형 물리 괴담에 통신 왜곡을 부여하지 마라).
...
## 출력 JSON 스키마
{
  ...
  "entity": {
    "name": "",
    "type": "매개형",
    // ↑ ★행동양식(축B) 1개★ — 괴담이 작동하는 기제(아래 '괴담 행동양식' 목록 중 하나). 성향(disposition)과 별개.
    "rules": [],
    "weakness": "",
    "solution": "",
    "exploit_path": "",
