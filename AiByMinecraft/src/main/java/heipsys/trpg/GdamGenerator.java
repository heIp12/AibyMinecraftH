...
- critical: true (중요 NPC): 독립 AI 인스턴스로 자율 행동. 스테이지당 1~2명으로 제한.
  * personality: 성격·숨겨진 면 한 문장 — ★상투형 라벨로 때우지 마라(겁쟁이·친절·예민 같은 유형어만 쓰지 말 것). 이 인물만의 구체적 모순·습관·디테일로 잡아라★ (예: "겁쟁이지만 비밀을 안다"처럼 뭉뚱그리지 말고 → "남 앞에선 침착한 척하지만 혼자되면 문을 세 번 확인하고, 열쇠를 쥐고도 모른 척한다"처럼). ★말투는 쓰지 마라(말투는 speech_style 전용)★.
