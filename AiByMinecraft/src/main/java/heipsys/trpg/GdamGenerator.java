...
""";
    ...
    private CompletableFuture<String> generateFamiliarConcept(int roomNumber, String filter) {
        ...
        switch (filter == null ? "random" : filter) {
            ...
            case "projectmoon" -> { scope = "프로젝트 문(로보토미 코퍼레이션·라이브러리 오브 루이나)의 환상체(Abnormality)";
                criterion = "★실존 환상체만 — 새로 지어내지 마라. 아래 실제 환상체 목록에서 고르거나, 네가 확실히 아는 다른 실존 환상체만 택하라(불확실하면 목록에서):\n"
                    ...
                    + "작업(본능·통찰·애착·억압)·E.G.O·격리 붕괴 조건 등 규칙·약점이 뚜렷한 환상체를 택하라.\n"
                    + "★이번 스테이지 위험 등급대(" + pmGradeBand(roomNumber) + ")에 ★맞는★ 환상체를 골라라 — 1~2스테이지엔 ZAYIN·TETH급 저위험(한 죄악과 수백의 선행·벌하는 새·그을린 소녀 등)을, 백야·종말조·마법소녀 같은 ALEPH(최상위)급은 ★후반 스테이지에만★ 등장시켜라."; }
    ...
    private static String projectMoonDirective(int roomNumber) {
        return "\n\n【프로젝트 문(로보토미 코퍼레이션) 전용 지침】 ★반드시 준수\n"
            + "1) ★환상체 이름 표기★: 등장하는 환상체(entity 포함)의 이름은 '변형명(원본 환상체 아종)' 형식으로 쓴다 — 예: 뽀삐(삐삐 아종), 백야(화이트나이트 아종). 정전 그대로 쓸 땐 정식 명칭 그대로.\n"
            + "2) ★원본 식별 정보 1개 이상★: 그 환상체의 원본이 무엇인지 알 수 있는 관찰 보고서·격리 기록·직원 증언(clues 또는 npcs.hidden_info) 최소 1개.\n"
            + "3) ★위험 등급★: entity·주요 환상체에 위험 등급을 표기한다(약→강: ZAYIN·TETH·HE·WAW·ALEPH). " + pmGradeBand(roomNumber) + ".\n"
            + "4) ★로보토미 본사 배경 가능★: 무대를 '환상체를 격리·관리하는 시설(로보토미 코퍼레이션 지부 등)'로 삼아 격리 붕괴·탈출·재격리·관리 업무를 사건 축으로 둘 수 있다.\n"
            + "5) ★전투표상·E.G.O. 기프트★: 플레이어 능력·도구(start_item·key_items)를 '전투 E.G.O.(전투표상)'·'E.G.O. 기프트'(환상체에서 추출한 무기·방어구·가호) 개념으로 이름·효과를 붙인다.\n"
            + "6) ★다수 환상체·관리직★: 한 시나리오에 여러 환상체가 동시에 등장할 수 있고, 관리직 NPC(세피라(말쿠트·예소드 등) 또는 관리자)가 방송·지시로 직원(플레이어)을 통솔하는 구도도 가능하다(방송 사건 활용).";
    }
