...
- 억눌린 분노형: 평소 잠잠하다 도발·저항·규칙위반에 ★폭발적으로 과격하게★ 반격(자극할수록 위험).
- 소극적 생존형: 먼저 안 나서고 ★숨고 피하며★ 자기 존재·약점이 위협받을 때만 반격(탐색 자체가 압박).
- 은밀한 야심가형: ★장기 계략★ — 조용히 세력·조건을 키우고 정보를 모아 결정적 순간에 판을 뒤집는다(직접 충돌 회피·조종 선호).
- 기회주의 사냥꾼형: ★약한·고립된 표적★의 빈틈을 친다(뭉치면 물러나고 흩어지면 덮친다).
- 집착형: 특정 대상 ★하나에 집착★해 끈질기게 쫓는다(나머지는 뒷전).
- 유희형: ★갖고 노는★ 타입 — 희망을 줬다 뺏고 함정으로 골린다(즉살보다 공포 연장).
- 심판자형: 자기 ★규칙·금기를 어긴 자만★ 응징(지키면 비교적 안전, 어기면 가차없음).
- 굶주린 포식자형: ★직선적·맹렬★ — 닥치는 대로 사냥, 속도·압박이 강점.
- 지능형: 플레이어의 ★패턴·습관을 학습★해 대응을 진화시키고 함정을 설계한다(가장 까다로운 두뇌형).
- 복수·원한형(한): 자기 죽음·배신의 ★원한★을 풀려 한다 — 원한 대상·연루자를 노리되 ★해원(잘못 바로잡기·진실 규명)으로 비폭력 해결★ 여지가 있다.
- 무차별·재앙형: 의도가 없다 — ★무심한 자연재해★처럼 번지고 삼킨다. 협상·도발이 안 통하고 회피·이탈만이 답(공포의 본질은 무관심).
- 모성·보호형: 대상을 ★"지켜준다"며 가두고 박제★한다. 보호 대상은 플레이어 또는 ★특정 NPC★일 수 있다(적대세력이 그 NPC를 노림). '그 NPC를 죽이면 끝난다'는 겉보기 해법이 함정·딜레마일 수 있다(방어막·제물·무고).
- 공포섭취형: ★공포를 먹고 강해진다★ — 일부러 공포를 극대화. ★평정 유지가 무기★(겁먹을수록 강해짐).
- 수집·완성형: 특정 ★세트(얼굴·이름·영혼·부위)를 모아 완성★하려 한다. 수집 기준에 걸린 자가 표적 — 네가 빠진 조각일 수도.
- 사육·이용형: 죽이지 않고 ★가둬 길러 수확★(공포·기력·노동)한다. ★생존 ≠ 안전★ — 사육장을 깨고 나가야 한다. (가두는 3형 구분: 모성=박제·보존 / 사육=수확·이용 / 수집=세트 완성.)
- 비탄·전염형(슬픔): ★절망·비탄을 전염★시킨다 — 곁에 두면 무력감·자해 충동. 희망·유대를 지켜 동화되지 않기.
- 정주형(원래 있던 것): 특정 ★구역에 묶여 늘 거기 있던★ 존재. 찾아오지 않고 그 구역에 들어온 자에게만 반응 — ★그 구역을 피하면 안전★(단 단서·풀이가 거기 있으면 들어가야 하는 딜레마). (소극적 생존형과 구분: 생존형은 어디든 따라다니며 위협 시 반격, 정주형은 그 구역에서 안 나온다.)
- 겁먹은형(공포에 질린 괴담): 괴담 ★자신이 겁에 질려★ 있다 — ★달래거나(소통·진정) 없애기★. 겁먹은 짐승처럼 발작적으로 위협할 수도(공포섭취형과 정반대).
- 주민형(의뢰·마을형): 괴담이 ★평범한 NPC처럼★ 행동한다 — 괴담의 의뢰를 해결하거나 ★겁먹지 말고 평범하게 대하기★가 풀이. 반대로 플레이어의 반사회적 행동(불법침입·기물파손·도둑질·폭행)이 트리거.
...
- 직접형(물리): 물리력·접촉으로 위협(할퀴기·짓이김·추격). 물리 제압·내성 책정 대상.
- 매개형(물체·기기): 전화·거울·사진·인형·통신 등 ★매개체를 통해서만★ 작동·이동·인식.
- 환경동화형: ★우리 세계의 환경에 잠복★(벽·그림자·물·군중) — 어디 있는지 모름. (★환경침식형(축A 규칙)과 구분: 여긴 세계는 멀쩡하고 괴담만 숨는다★)
- 인지·정신형: ★감각·기억·정신을 상시 왜곡★(환영·환청) — 보이는 걸 못 믿음(세계 자체는 멀쩡).
- 음향·소리형: ★소리·목소리·이름 부름★으로 작동·전파(들으면 위험, 침묵이 방패).
- 정보·언어형: ★단어·문자·소문·앎★이 통로(말하거나 알수록 작용).
- 모방·거울형: ★복제·반사·위장★ — 사람·목소리·모습을 흉내 내 섞여든다.
- 기생·숙주형: ★살아있는 숙주(사람·동물)를 갈아타며★ 이동·확산(누가 숙주인지 모름·숙주로 위장).
- 세계창조형: 일상에서 괴담 ★고유의 세계로 끌어들이는 경로★(들어가면 못 나오는 저택·거울 너머) — 그 안에선 괴담이 토착민, 우리가 이방인. (★비지구무대형(축A)과 구분: 비지구무대형은 처음부터 무대가 이세계인 '규칙', 세계창조형은 그 세계로 '끌고 가는 기제'★)
- 꿈·무의식형: ★수면·환각 상태에서만★ 괴담의 영역에 진입(깨면 안전하나 잠을 안 잘 순 없다).
