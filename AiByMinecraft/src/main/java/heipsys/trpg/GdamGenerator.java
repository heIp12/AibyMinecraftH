...
1. ★완급 + 사이드 사건(작은~중간 규모의 독립 사건)★: 큰 본선 사건 사이에 ★곁가지 사건(side:true)★을 끼워라 — 분위기 환기·자원 소모·짧은 선택. 사이드도 사실상 작은 괴담처럼 ★자체 규모(side_scale: "small"|"medium")·발생/마감 시각(time·deadline)·가벼운 성격(disposition)·자체 전개★를 가질 수 있다(전부 본선에 직결될 필요는 없다).
