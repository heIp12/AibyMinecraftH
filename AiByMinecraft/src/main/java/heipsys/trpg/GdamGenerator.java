...
""";
        ...
        switch (filter == null ? "random" : filter) {
            ...
            case "projectmoon" -> {
                ...
                criterion = "★실존 소재만 — 새로 지어내지 마라. 아래 실제 환상체 목록에서 고르거나, 네가 확실히 아는 다른 실존 환상체/라이브러리·도시 소재만 택하라(불확실하면 목록에서):\n"
                    ...
                    + "\n★이번 회차 무대(지정): " + pmStage + " — 반드시 이 무대로 구성하고 세계관 어휘를 거기에 맞춰라(로보토미로 되돌리지 마라).\n"
    ...
    private static String projectMoonDirective(int roomNumber) {
        return "\n\n【프로젝트 문 전용 지침】 ★반드시 준수 — 로보토미 한 세팅에 갇히지 마라\n"
            + "0) ★무대 다양화(최우선)★: 프로젝트 문의 무대는 '도시(The City)' 전체다. 아래 셋 중 ★이번 시나리오에 맞는 하나를 골라 다양하게★ 써라(매번 로보토미 격리시설로만 만들지 말 것):\n"
            + "   (A) 로보토미 코퍼레이션 — 환상체 격리·관리 시설(격리 붕괴·작업·E.G.O.).\n"
            ...
            + "   ※ 이번 무대를 (A)(B)(C) 중 ★직전 회차와 다르게★ 골라, 시나리오·세계관 어휘를 거기에 맞춰라.\n"
