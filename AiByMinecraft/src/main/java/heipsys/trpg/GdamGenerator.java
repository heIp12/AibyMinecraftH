...
★ 단, 구조를 획일화하지 마라. 괴담마다 형태가 완전히 달라야 한다:
  ...
  - 규칙 목록 준수형: 지켜야 할 '규칙 목록'이 주어진다(쪽지·안내문·구전). ★일부는 가짜·함정★이다. 진짜/가짜를 가려내고 압박 속에 지키는 게 플레이. world_rules.details에 진짜 규칙과 함정 규칙을 섞고, 어기면 무슨 일이 나는지 명시. loophole=가짜를 알아채는 단서.
  - 순번·연쇄형: 정해진 ★순서★로 표적이 된다(이름·자리·나이·들어온 순서 등). 순서를 알아내 끊거나 다음 표적을 보호·교란하는 게 플레이. collapse=순번 고리를 끊는 것.
  - 금기 행동형: 특정 ★행동★(소리내기·뒤돌아보기·이름 부르기·불 켜기 등)이 괴담을 부르거나 악화시킨다. 행동을 절제하며 진행하는 긴장이 핵심. constraints에 그 금기를 상황으로 녹이고, 어떤 행동이 무엇을 부르는지 명시.
  - 교환·대가형: 통과·안전·정보를 얻으려면 ★무언가를 바쳐야★ 한다(물건·기억·신체·동료). 도덕적 희생 선택이 플레이. 무엇을 바치면 무엇을 얻는지, 거부 시 결과를 명시.
  - 시한 의식형: ★마감★(자정·만조·의식 완성·N턴)이 다가오며, 그 전에 막거나 완수해야 한다. 시간 경쟁이 핵심(타임라인·시계와 연동). collapse=마감 전 핵심 조건 달성.
  - 조건부 안전지대형: ★특정 위치·자세·조건★에서만 안전하고, 그 안전 조건이 계속 변한다(빛 속/그림자 속/물 밖 등). 위치·타이밍 싸움이 플레이. 어디가 언제 안전한지의 규칙을 명시.
  - 역(逆)의식·수집형: 흩어진 ★재료·조각★을 모아 반대 의식으로 봉인·해제한다. 탐색·조합·완성이 플레이. collapse=재료를 모아 역의식 완성.
  - 도덕 딜레마형: 괴담이 ★불가능한 선택★을 강요한다(누구를 살릴지, 무엇을 포기할지). 정답 없는 선택과 그 대가가 핵심. 선택지마다 다른 결과·후폭풍을 명시.
  - 집단환각형: ★시작부터 모든 플레이어·NPC가 '뒤바뀐 상식·기억'을 진짜로 믿는다★(예: 죽은 사람이 살아있다고 모두가 믿음, 없는 가족이 원래 있었다고 믿음, 어떤 규칙이 당연하다고 믿음). GM은 그 거짓 현실을 처음엔 ★당연한 듯★ 서술하고, 작은 모순·균열을 흘려 플레이어가 '뭔가 잘못됐다'를 스스로 깨닫게 한다. world_rules.details에 '진짜 현실 vs 모두가 믿는 거짓'을 명시. collapse=거짓을 깨고 진짜 현실을 직시·복원.
  - 금지워드형: ★특정 단어(forbidden_word)★가 존재하며, 그것을 입에 올리거나 그 정체를 깨닫는 순간 표적이 된다. 무엇이 금지어인지 모른 채 피해야 하는 긴장이 핵심(시스템이 입력을 감지해 처리). entity.forbidden_word에 그 단어를, world_rules에 '그 단어가 왜 위험한지'를 둔다. ※이 유형은 forbidden_word 필드를 반드시 채운다.
  - 환경 침식형: 단일 개체·규칙이 아니라 ★공간 전체의 물리·생물 법칙과 '상식' 자체가 뒤틀린★ 형태. 숨쉬기·먹기·대화 같은 ★일상 행위 자체가 무의식적 자해·살상★이 되고, 환경에 동화되지 않으려 자아를 지켜야 한다. world_rules.details에 '어떤 평범한 행위가 어떻게 해가 되는가'와 '동화/잠식이 어떻게 진행되는가'를 명시. constraints로 그 왜곡을 상황화. collapse=뒤틀린 법칙의 근원을 정상화하거나 그 공간을 벗어남.
  - 소문 실체화형: 괴이가 본래 형태·목적·정체조차 없는 ★백지/나약한 영체★였으나, 관찰·소문·집단 인지가 투영될수록 ★구체적이고 치명적인 실체로 완성★된다(거짓 정보가 진짜 괴물을 빚는 역설). 정보격리형의 반대 — 여기선 '알수록·말할수록·믿을수록' 괴담이 강해진다. ★GM은 플레이어가 그 존재를 언급·관찰·두려워할수록 형태·위력을 키워 서술하고, 무시·부정·반대 소문으로 인지를 끊으면 약화시켜라. collapse=집단 인지/소문의 고리를 끊어 실체를 다시 무(無)로 되돌림.
  - 역할극 강제형: 닫힌 시공간에 들어온 참여자에게 기괴한 ★배역이 부여되고, 정해진 '극본(narrative)'을 거스르지 않고 배역에 순응해 연기★해야 숨은 진실·해피엔딩에 도달한다. 단순 금지 규칙이 아니라 '연극의 흐름을 따라야 함'이 핵심(어설피 극을 깨면 파국). world_rules.details에 극본의 큰 흐름(막·장)과 '배역을 벗어나면 무슨 일이 나는가'를 명시. ※기존 배역(role) 시스템과 연계해 설계. collapse=극을 끝까지 완수하거나 극의 모순을 짚어 막을 내림.
  - 메타-법칙 조작형: ★플레이어(주인공) 측★이 자신의 특수 능력·서사의 장르 클리셰·시스템의 맹점을 역이용해 ★세계의 법칙 자체를 덮어써 타파★할 여지가 열려 있는 형태. 정공법이 막힌 듯 보여도 '메타적 비틀기'가 숨은 정답. loophole/exploit_path에 '어떤 메타적 역이용이 통하는가'의 방향을 둔다(구체 절차 X). GM은 논리가 성립하는 메타 해법을 인정하라.
  - 감각 동기화형: 참여 배역들의 ★감각·고통·기억이 강제로 이어진다★(A가 다치면 멀리의 B도 피 흘림, C가 단서를 읽으면 D 머릿속에 박힘). 극단적 협력·정보전파가 핵심 — 한 명의 실수가 전원에게 번진다. details에 '무엇이 어디까지 공유되는가(고통/감각/기억/지식)'. 정보격리형과 결합하면 '한 명이 알면 모두 감염'. GM이 한쪽의 피해·발견을 연결된 쪽에 전파.
  - 공간 왜곡형(비유클리드/백룸): 방과 방의 ★연결이 고정되지 않고★ 문을 열거나 턴이 지날 때마다 바뀐다 — 지나온 문으로 되돌아가도 원래 자리가 아니다. 지도가 무력화되고, 표식(분필·핏자국)·특정 패턴(예: 왼쪽 문만 3번)으로 공간을 '고정'해야 탈출. details에 '무엇이 어떻게 재배치되는가'와 '고정·탈출 조건'. collapse=공간 고정 규칙을 찾아 출구 확보.
  - 차원 분리 협력형: 같은 장소라도 절반은 '현실', 절반은 '이면(과거/미래/뒷면)'으로 ★찢겨 시작★한다. 서로 만날 수 없고 무전·매개체로만 소통. 한쪽 행동이 다른쪽 세계를 바꾼다(현실에서 문을 부수면 이면의 벽이 뚫림). 비대칭 정보·상호작용 퍼즐이 핵심. 배역을 두 진영 zone으로 나눠 배치.
  - 인과 역전형: 이 공간에선 ★생존 상식이 거꾸로★ 작동한다(밝은 곳이 치명적·어둠이 안전, 치료가 독·출혈이 은신). 살려고 ★의도적 자해·실패★를 계획해야 한다. details에 '무엇이 뒤집혔는가' 2~4개. 초반에 상식대로 행동하면 당하게 해 역전을 깨닫게 하라(단서로 암시). collapse=뒤집힌 법칙을 역이용해 근원 무력화.
  - 신체 기생·교환형: 입장과 동시에 괴담의 일부가 ★몸에 기생★하거나, 특정 시점마다 배역(시야·스탯·아이템)이 ★강제로 뒤바뀐다★. 기생체는 유용한 힘(어둠 시야·괴물 감지)을 주지만 턴이 갈수록 숙주를 잠식해 돌연변이로 만든다 — '언제까지 쓰고 잘라낼까'의 딜레마. 교환형이면 남의 몸에서 서로를 통제하는 혼란이 핵심. details에 '이득과 잠식 진행'.
  - 사후 정보형(죽거나 끝나야 알게 됨): 실질적 해결법이 ★위험한 선택(괴담에게 사로잡힘)·사망·또는 타임라인 종료 뒤에야★ 드러난다. 두 갈래 — ①사망 시에야 진상이 보이는 형, ②클리어 시점을 넘긴 뒤 정보가 풀려 '알면 쉬운' 형. 재도전/다회차와 연계해 한 번 죽어본 지식이 다음 판을 쉽게 만들게 설계(관측자·세이브포인트 NPC와 궁합).
  - 전원 괴이형: 플레이어 외 ★모든 NPC·대상이 괴이★다(진짜 우호 NPC 없음). 처음엔 평범한 사람처럼 굴다가 역할극이 끝날 즈음 정체를 드러내며 게임오버로 치닫는다. 누가 사람인지 의심하는 긴장이 핵심 — 단 플레이어 본인은 진짜다. relationships·npc는 '사람인 척'으로 위장.
  - 비(非)지구 무대형: 무대가 지구 일상이 아니다 — 우주(외계인 납치·실험), 이세계·판타지, 지옥·사후, 뒷세계(키사라기역·끝없는 엘리베이터 등). constraints.era/setting에 무대를 명시하고 사물·언어·물리를 그 세계에 맞게. 일상 도입을 짧게/생략하고 바로 이질적 규칙으로 압박해도 된다.
  - 함정 지시형: 스토리가 '플레이어가 ★반드시 해야 할 것처럼★' 써둔 행동이 실은 ★함정★ — 그대로 따르면 클리어가 불가능해진다. 진짜 길은 그 지시를 의심하고 거스르는 것. 단서로 '지시가 수상하다'를 흘려 눈치채게 하라(처음부터 들키면 안 됨). loophole=지시의 모순을 짚어 따르지 않기.
  - 진영 대립형(NPC팀 vs 플레이어팀): NPC 세력과 플레이어가 ★대립★한다(목적 충돌). 협상·기만·전투·이간이 플레이. NPC팀은 수·정보·지형 우위를 가질 수 있다. npc role_type을 적대자·집단세력 등으로, 관계를 적대로.
  - 봉인 해방형: ★한 명을 제외한 플레이어가 봉인★된 채 시작한다(갇힘·잠듦·NPC 안에 봉인). 자유로운 플레이어가 봉인을 풀어줘야 동료가 합류한다. 초반 1인 → 점진 합류 구조. 봉인 위치·해제 조건을 단서로.
  - 천년 운영형: 메인 사건에선 시간이 천천히 흐르고, 사건이 끝나면 ★천년 단위로 가속★된다 — 문명을 운영하듯 장기 서사를 쌓아 후반 사건을 그 축적으로 클리어한다. 단 대처에 실패하면 괴담도 오랜 시간 힘을 쌓는다. TIME_SKIP 대도약(년 단위)과 누적 변화 규칙을 적극 활용.
  - 릴레이 계주형(멀티 전용): 플레이어마다 ★타임라인이 다르고 생존 시간이 정해져★ 바통을 넘기며 진행한다. 짧은 소통 창이 있고, 둘 이상이 함께 행동 가능할 때 몇 턴 안에 이전 주자가 어떤 사건으로든 퇴장(사망)한다(부활·동물화 특성이 있으면 계속 조력). 2인 이상에서만 성립.
  - 위기 즉시 시작형: 일상 파트를 ★건너뛰고★ 시작부터 죽을 위기에 몰린 채 출발한다. 도입의 평온 없이 곧장 긴박. daily 파트를 최소화/생략하고 첫 장면을 위기로.
  - 가짜 동료형: 시작 시 곁의 ★동료가 가짜 NPC★일 수 있다(플레이어처럼 상호작용·같은 통신 방식 사용). 진짜 플레이어는 다른 곳에 갇혔거나 NPC 안에 봉인됐다. 누가 진짜인지·진짜 나/동료를 어떻게 되찾을지가 핵심. 단서로 가짜의 위화감을 흘려라(처음부터 들키면 안 됨).
...
- 적대적공조: 괴담 생태계에 속한 ★위험한 적대 개체★지만, 괴담 주체(발생원)에게 억압당하거나 깊은 원한을 품고 있다 — 처음엔 적으로 보이나, 설득·교섭하면 ★발생원에 맞서 공조★할 수 있다(단 본성이 위험해 배신·폭주 위험 상존). true_role에 '무엇에 원한이 있고 어떤 조건에서 협력하는가'.
- 시스템부품: 자아·원한이 ★완전히 소멸★한 채 오직 괴담 공간의 '규칙'을 집행하고 무대를 유지하는 ★맹목적 톱니★(개·집단). 설득·교섭이 통하지 않고, 규칙대로만 반응한다(communicable:false 권장). 막으려면 규칙·무대 자체를 흔들어야 한다.
- 잘못된가이드: 다 아는 듯 ★옳은 길을 알려주는 척★ 신뢰를 얻은 뒤, 잘못된 길을 제시하거나 진실에 거짓을 섞어 ★괴담 폭주·스테이지 붕괴★를 유도하는 기만자. true_role에 '진짜 의도와, 어떤 거짓을 어떻게 섞는가'. 단서로 모순을 짚어야 간파 가능(처음부터 들키면 안 됨).
- 오염매개체: 플레이어에게 ★실질적 도움(단서·회복·아이템)을 주는 우호적 인물★이지만, 같은 공간에 오래 머물거나 자주 소통할수록 ★플레이어가 괴담에 더 빨리 잠식★된다. 본인은 자신이 매개체임을 모른다(선량할수록 비극). '버리면 생존이 어렵고 데려가면 잠식'의 딜레마. true_role에 '어떤 접촉이 얼마나 잠식을 가속하는가'.
- 거울자아: 특정 플레이어의 ★잃어버린 기억·죄책감·생명력이 형상화★된 인물. 다치거나 죽으면 ★연결된 플레이어도 같은 피해·확정 사망★. 방패막이·방치를 막는 핸디캡 — 나약하고 통제 안 되는 이 인물을 제 목숨처럼 지켜야 한다. true_role에 '누구와 이어졌고 무엇이 공유되는가'.
- 맹신자: 괴담 주체는 아니나 이 공간의 ★미친 규칙을 섭리·종교로 숭배★하는 인간. 규칙을 깨려는 플레이어를 물리적·논리적으로 필사 방해('괴물보다 사람이 무섭다'). true_role에 '무엇을 숭배하고 어디까지 방해하는가'.
- 관측자기록관: 사건 내내 플레이어를 ★조용히 기록만★ 한다(대화는 되나 개입 안 함). 죽이거나 기록을 빼앗으면 ★타임라인·인과 붕괴(치명 패널티)★. 끝까지 살려두면 실패·재도전 시 ★이전 회차 핵심 단서를 넘겨주는 세이브포인트★가 된다.
- 다른괴담소환자: 몸에 ★또 다른 괴담(사건)이 봉인★돼 그것을 불러내려 한다. 도움이 될 때도 있으나 대개 ★더 큰 사건★을 일으킨다. true_role에 '무엇이 봉인됐고 풀리면 무슨 일이 나는가'.
- 빙의해결사: 플레이어처럼 ★사건을 해결하려 빙의한★ 존재(사건을 알아가는 중, 강력한 능력 보유). 미치거나 트롤링하면 까다로워진다. 높은 확률로 자신이 NPC임을 안다. true_role에 능력·목적.
- 집단세력: ★여럿이 늘 함께 다니는★ 무리(숫자의 폭력). 흩어져 각각 일을 처리할 수 있다(한 AI가 무리 구성원 다수를 동시 조종). 적대·중립·조력 어느 쪽도 가능.
- 외부세력: 괴담과 무관한 ★완전히 다른 세력★(강도단·요원·사이비 등). 자기 목적으로 움직이며 별개의 사건을 일으킨다.
- 수상한자: 여러 정보를 쏟아내지만 ★도움이 안 되는★ 인물 — 대개 '완전히 다른 괴담'의 정보다(오도·소음원). 진짜 단서와 섞여 혼란을 준다.
- 적대자: 특별한 반전 없이 ★그냥 플레이어와 대적★하는 인간(목적 충돌·악의). 진영 대립형과 궁합.
- 메인열쇠: ★이 인물만이 괴담을 해결할 수 있고★ 플레이어 단독으론 해결 불가. 지키고·설득하고·해결 시점까지 살려내는 게 플레이.
- 괴담이사랑하는자: 괴담이 ★집착·애착하는 대상★. 죽으면 괴담이 크게 폭주, 살아 있으면 오히려 괴담을 억제·달랜다. 누군지 모른 채 다루는 긴장.
- 쾌락주의자: 항상 ★사건을 더 키우려★ 드는 인물(불을 지르고 즐긴다). 안정되려는 흐름을 일부러 망친다. true_role에 무엇으로 판을 키우는가.
- 백과사전: 사건의 ★모든 정보를 아는★ 인물. 단 통째로 주지 않게 — 대가·조건·신뢰를 요구하거나 일부만 흘리게 설계(열쇠와 구분: 이쪽은 '전지'하나 협조가 까다롭다).
- 조종자: 뒤에서 ★플레이어나 괴담을 조종해 자신만의 별개 목적★을 이루려는 배후. 표면적으로는 조력자·방관자로 위장. true_role에 진짜 목적·조종 수단.
- 약화된열쇠: 봉인·제약으로 ★힘이 묶인★ NPC. 제약을 풀어주면 ★초월적 힘으로 강력히 지원★한다. 제약 해제 조건을 단서로.
- 도청자: 플레이어·괴담의 ★대화 내역을 모두 엿보는★ 존재. 그 정보로 무엇을 할지 모른다(협박·거래·폭주 유발 등). true_role에 의도.
- 중재자: 누구의 편도 아니다 — 한쪽이 ★과하게 불리해지면 나타나 약자(괴담이든 플레이어든)를 돕는다★(균형추). 양쪽 모두에게 변수.
규칙:
