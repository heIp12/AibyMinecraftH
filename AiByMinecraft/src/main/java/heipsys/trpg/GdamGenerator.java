...
- ★AI 창작이 아닌 '실존 괴담' 기반이면(컨셉에 [친숙한 괴담 모드]·출처·원전이 있으면) entity.name은 ★플레이어가 나중에 실제로 검색해 원전 정보를 찾을 수 있는 '한국에서 부르는 이름'★으로 짓는다:
  · 한국에서 통용되는 한국어 이름이 있으면 → "한국어이름(원어 원문)" 형식. 예: "우는 천사(The Weeping Angel)", "입 찢어진 여자(口裂け女)".
  · 한국어 이름이 없고 음차로만 불리면 → "원어 원문(한국어 음차)" 형식. 예: "The Weeping Angel(위핑엔젤)", "Backrooms(백룸)".
  · 단, 원전이 본래 한국 것(원어가 한국어)이면 괄호 없이 그 한국어 명칭만 쓴다. 예: "장산범", "빨간마스크".
  · 목적은 ★검색 가능성★이다 — 실존 명칭을 보존하고, 임의 재번역·창작 이름으로 원전을 훼손하지 마라. 괄호가 필요한 경우 원문/음차를 빼먹지 마라.
