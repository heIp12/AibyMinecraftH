...
3. ★체력·근력이 쓰이는 물리 위기 사건★: 최소 1개는 ★완력·체력·무기가 실제로 통하는 위기★를 넣어라(combat:true). 본체가 물리내성이라도 ★하수인·변이체·구울·풀려난 잡것·적대 인간 등 '싸울 수 있는 대상'★을 등장시켜 힘으로 돌파·방어하게 하라(예: 뱀파이어 괴담 → 구울 무리 습격, 봉인형 → 풀려난 잡것들). ※ 지금 체력·근력 스탯이 거의 안 쓰이는 문제를 이 사건으로 해결한다. (단 순수 정신·회피형 괴담이면 무리한 전투 대신 도주·은신 압박 사건으로 대체.)
