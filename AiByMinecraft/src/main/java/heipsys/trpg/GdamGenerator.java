...
""";
    ...
    public CompletableFuture<JsonObject> generate(int roomNumber, boolean familiar, Consumer<String> progress) {
        ...
        return generateFamiliarConcept(roomNumber).thenCompose(concept -> {
            logger.info("[gdam] 친숙 모드 — " + concept.lines().filter(l -> l.startsWith("이름:")).findFirst().orElse("?"));
            if (progress != null) progress.accept("컨셉");
     ...
     */
    private CompletableFuture<String> generateFamiliarConcept(int roomNumber) {
        ...
        String task = "너는 전 세계 괴담·도시전설·민간전승을 꿰뚫는 큐레이터다.\n"
            + "다음 지역/계열에서 '실제로 전해지는' 대표적이고 잘 알려진 괴담·도시전설 1개를 골라라:\n"
            + "→ " + region + "\n"
            + avoid
            + "마이너·모호한 것 말고, 규칙과 약점이 뚜렷한 괴담을 택하라. 거울·반사·모방 소재는 금지.";
        ...
        return aiManager.callAssistant(task, data).thenApply(raw -> {
            ...
            if (lore.isBlank()) return fallbackFamiliarConcept(roomNumber);
        ...
        }).exceptionally(ex -> fallbackFamiliarConcept(roomNumber));
    }

    /** AI 선택 실패 시 큐레이션 목록에서 1개 사용 (안전 폴백). */
    private String fallbackFamiliarConcept(int roomNumber) {
