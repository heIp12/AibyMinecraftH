...
- critical: true (중요 NPC): 독립 AI 인스턴스로 자율 행동. 스테이지당 1~2명으로 제한.
  ...
  * speech_style: 이 인물의 목소리 — 서술형 ★딱 한 문장(40~70자)★. 어조·리듬 한 구절 + 입버릇 ★한 가지★를 '경향'으로 녹여라(예: "느릿하게 뜸을 들이며 '뭐랄까'를 자주 붙인다") — 버릇은 한 인물에 ★한 종류만★, 여러 말투·버릇을 복합하지 마라. 어미를 언급하면 '-거든(요)'처럼 존/반 중립 기본형으로 — ★존댓말/반말은 지정 금지★(게임이 상대 나이·관계로 그때그때 정한다). ★개성 어미(문장 끝 고정 어미)는 여기 speech_style에 넣지 마라 — 그건 아래 ★ending_style★ 필드로 따로 지정한다(그 NPC만 출력 후처리로 어미를 확실히 렌더한다). speech_style은 어미가 아니라 어조·리듬·필러 같은 '결'만 담당하라.★★금지★: 어미 나열 목록 · 예시 대사 · 말끝 흐림/토막말/수수께끼투(‘온전한 문장’ 원칙 위반) · 감정별 복수 말투 · 과한 사투리·연극체·문어체 · ★직무·보고체 접두어 버릇('중요한 말 앞에 「기록상」·「확인 결과」·「보고하자면」을 붙인다' 식 — 매 핵심 문장이 보고서처럼 되어 사람이 아니라 업무 봇으로 들린다; 접두어형 입버릇은 '뭐랄까·있잖아·음' 같은 인간적 필러만 — 직무·업무 용어 접두어 금지). 긴장·공포 시 변화는 캐릭터 정의적일 때만 조건절 1개("놀라면 말이 빨라진다" 식, 그래도 온전한 문장). ★NPC끼리 말버릇 중복 금지★(텍스트만으로 화자 구분이 목적). 나이·지능·직업에 맞게(어린이는 쉬운 말씨).
