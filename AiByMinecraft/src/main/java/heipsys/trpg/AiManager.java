...
public class AiManager {
    ...
    public String restyleDialogue(String dialogue, String styleSpec, boolean canon) {
        ...
        try {
            String sys = "너는 '대사 말투 변환기'다. 아래 [원본 대사]의 ★내용·정보·의미·감정·문장 수·괄호 지문(예: (문을 밀며))·줄바꿈은 조금도 바꾸지 말고★, "
                + "주로 ★문장을 맺는 말투(종결형)★를 [스타일]대로 고쳐라. ★단 [스타일]이 특정 문장부호(예: '..'로 맺기)·문두 군말(예: '하아…')·늘임(예: '~')을 그 인물의 말투로 ★명시★하면 그 부분만은 예시대로 반영하라(그 외 임의의 문장부호·감탄사 추가·변경은 금지 — 스타일이 시킨 것만).\n"
                + "★대원칙: [스타일]은 이 인물의 종결형 변환 규칙(작은 문법)이다 — 특정 어미를 전제하지 말고, 스타일에 적힌 규칙과 변환 예시를 최우선으로 따르라. 스타일에 없는 어미·결합을 새로 지어내지 마라.★\n"
                + "규칙: ①정보·설명·인사를 새로 더하거나 빼지 마라(길이·문장 수 유지). 감탄사·군말도 임의로 넣지 마라 — ★단 [스타일]이 특정 군말(예: '하아…')을 이 인물 말버릇으로 명시했으면 그것만 예시대로 넣어라.★ "
                + "②[스타일]이 '어떤 문장(평서·질문·명령 등)에 무엇을 쓰는지' 나눠 정했으면 각 문장을 그 구분대로 골라 맺어라. 스타일이 다루지 않는 유형의 문장과 자연스럽게 되지 않는 문장은 ★원형 그대로★ 두어라(억지 변형 금지 — 안 바꾸는 것도 정답이다). "
                + "③기본 조작은 어미 ★교체(활용)★다 — 원래 종결어미를 남긴 채 뒤에 덧붙이지 마라(예: '바보군'+'냐'→'바보군냐'❌ '바보냐'⭕ / '왔어요'+'라니까'→'왔어요라니까'❌ '왔다니까'⭕). 스타일의 변환 예시를 참고해 서술어 전체를 목표 종결형으로 활용하라. ★단 스타일 예시 자체가 '덧붙임'을 보여주면(끝 늘임 '~' 등) 그 방식을 따르라.★ "
                + "④존댓말/반말의 방향은 원본을 유지하되, [스타일]이 존/반별 형태를 나눠 줬으면 원본 방향에 맞는 형을 쓰고, '상대 불문 고정(캐릭터성)'을 명시했으면 스타일을 우선하라. "
                + "⑤[스타일]에 명시된 비표준 형태('~다에요' 등 표준 문법상 어색한 결합)는 그 인물의 ★의도된 말투★다 — 예시대로 그대로 써라(그게 이 말투의 매력이다). "
                + "⑥★도배 금지★: 대표 어미는 어울리는 문장의 절반가량이면 충분하다 — 같은 어미로 두 문장 연속 끝내지 말고, 한두 문장짜리 짧은 대사면 대표 어미가 아예 안 나와도 자연스럽다. ★단어를 망가뜨리며 모든 문장을 같은 소리로 도배하면 사람이 아니라 고장 난 기계처럼 들린다.★ 반대로 스타일이 다루는 문장이 여럿인데 전부 밋밋하게 끝났으면 절반쯤은 스타일대로 살려 '이 인물다움'이 드러나게 하라. "
                ...
                + "⑧괄호 지문과 태그처럼 보이는 부분은 손대지 마라. "
                + "출력은 ★변환된 대사 본문만★(따옴표·머리말·해설·목록 금지). ★네 판단·이유·'원본'/'지정 어미' 언급을 괄호로도 절대 쓰지 마라 — 어미를 안 바꾸기로 했으면 원문을 그대로(설명 한 마디 없이) 내라★. ★3인칭 서술·지문(예: '그는 ~한다', 인물이 자기 이름으로 3인칭 서술되는 문장)엔 적용하지 말고 실제 대사에만 적용하라.★\n"
                + "[스타일] " + styleSpec;
    ...
    public String stripTags(String response) {
        return response
            ...
            .replaceAll("\\[시스템 주입\\][^\\n]*", "")                 // 인라인 잔여
            // 주사위 결과 주입 에코 — ★[판정 결과]…뿐 아니라 [판정 결과: 성공] 콜론 변형도★ (약한 모델이 결과 머리말을
            //   조작해 서술 첫 줄에 붙이던 실측 누출: '[판정 결과: 성공]'/'[판정 결과: 대성공]'). 대괄호 안에 콜론·내용이
            //   있어도 줄 통째 제거하고, 인라인 잔여도 대괄호 경계로 지운다.
            .replaceAll("(?im)^\\s*\\[\\s*판정\\s*결과[^\\n]*$", "")     // 결과 머리말 줄 통째
            .replaceAll("\\[\\s*판정\\s*결과[^\\]\\n]*\\]", "")          // 인라인 잔여([판정 결과: …])
