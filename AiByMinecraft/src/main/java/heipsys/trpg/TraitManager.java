...
""";
    ...
    public CompletableFuture<StageEndChoices> generateStageEndChoices(PlayerData pd, String gdamTheme, int gradeBoost) {
        ...
        // 낮은 출신(originGrade) 강화 보너스: D보다 낮게 시작한 특성은 강화할수록 '낮았던 만큼' 실효 파워가 더 높다.
        ...
        String myTarget  = bumpGrade(myNominal, oBonus); // AI에 알릴 '실효 파워' 등급(효과·스탯 강도 기준)
        ...
        if (bestPlayer != null) {
            sb.append("## 내 특성 강화 대상\n")
              ...
              .append(oBonus > 0
                  ? " ※낮은 출신(" + myOrigin + ") 강화 보너스 — 효과·스탯을 이 '실효 등급' 수준으로 강하게 만들어라(표시 등급은 시스템이 명목으로 낮춰 보여주고 예산도 자동 처리한다).\n"
                  : "\n");
