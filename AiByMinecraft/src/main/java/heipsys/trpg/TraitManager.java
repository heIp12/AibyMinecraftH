...
""" + (scenarioFlavor.isBlank() ? "" : "\n\n" + scenarioFlavor
    ...
    public CompletableFuture<StageEndChoices> generateStageEndChoices(PlayerData pd, String gdamTheme, int gradeBoost, int powerBonus, String maxGrade, boolean preferActive) {
        ...
        // ★EX 상한★: 강화 대상이 이미 S(또는 EX)면 강화가 EX까지 도달할 수 있게 상한을 EX로 연다 —
        //   신규 드롭은 여전히 maxGrade(≤S) 상한이라 EX는 'S 특성을 깊이 강화'한 경로로만 나온다(인플레 없음).
        boolean myAtTop  = bestPlayer != null && gradeToInt(bestPlayer.grade) >= gradeToInt("S");
        String myCap     = myAtTop ? "EX"
                         : (bestPlayer != null && gradeToInt(bestPlayer.grade) > gradeToInt(maxGrade)) ? bestPlayer.grade : maxGrade;
        String mapCap    = (bestMap    != null && gradeToInt(bestMap.grade)    > gradeToInt(maxGrade)) ? bestMap.grade    : maxGrade;
        // 실효 파워 상한도 myCap이 EX일 때만 EX 허용(그 외는 종전대로 S) — 약자보정이 비-S 특성을 EX로 밀지 못하게.
        String effCap    = clampGrade(bumpGrade(myCap, Math.min(2, Math.max(0, powerBonus))), gradeToInt(myCap) >= gradeToInt("EX") ? "EX" : "S");
...
- ★목표 등급이 "EX"면 최상위 티어다★ — S보다 한 단계 위(약 1.3배 위력·정밀도). S 특성을 깊이 갈고닦아 도달한 정점이니, 같은 정체성을 유지한 채 S보다 확연히 강하고 완성된 형태로 만들어라(수치·범위·효과를 S 대비 한 급 위로). 능동/수동 성격은 원본 그대로.
...
""" + SystemTraitRegistry.buildAiCatalog()
        ...
        return aiManager.callTraitGen(system, prompt).thenApply(raw -> {
            try {
                String cleaned = raw.replaceAll("```json", "").replaceAll("```", "").trim();
                int s = cleaned.indexOf('{'), e = cleaned.lastIndexOf('}');
                if (s == -1 || e == -1) return fallbackStageEndChoices(pd, fp, fm, maxGrade); // 보상 조용한 소실 방지
                ...
                return fallbackStageEndChoices(pd, fp, fm, maxGrade); // 파싱 예외에도 보상 보장(조용한 소실 방지)
            }
        });
    }

    /**
     * ★성장 생성 파싱 실패 시 보상 보장★: 예전엔 JSON이 조금만 깨져도 null을 반환해 클리어 보상이 조용히 사라졌다.
     * 저품질·다양한 모델(GPT가 필드를 설명문으로 감싸거나 중간에 끊는 등)에서 취약 → 결정론적 new_trait 1개를 지급한다.
     * (강화 후보는 생략하고 소소한 스탯 특성 하나만 보장 — 완전한 보상 손실보다 낫다.)
     */
    private StageEndChoices fallbackStageEndChoices(PlayerData pd, TraitData fp, TraitData fm, String maxGrade) {
        String[][] pool = {
            {"각성한 본능", "위기를 넘기며 몸에 밴 감각", "판정이 필요한 순간 살짝 유리하게 작용한다.", "spr"},
            {"버텨낸 뚝심", "고비를 견뎌 단단해진 마음",   "힘든 순간에도 쉽게 무너지지 않는다.",     "str"},
            {"살아남은 눈치", "여러 번 죽을 뻔하며 길러진 촉", "위험한 낌새를 조금 더 빨리 알아챈다.",   "luk"}
        };
        String[] p = pool[Math.floorMod(pd.traits.size(), pool.length)]; // 플레이어별 결정론적 회전(구 기여도 기반 → 특성 수 기반)
        JsonObject o = new JsonObject();
        o.addProperty("name", p[0]);
        o.addProperty("description", p[1]);
        o.addProperty("effect", p[2]);
        o.addProperty("active", false);
        o.addProperty(p[3] + "_add", 1);
        String g = clampGrade("C", maxGrade);              // 스테이지 상한 내 소소한 보상(보통 C)
        TraitData newT = parseStageEndTrait(o, g, null);    // 기존 파서 재사용(applyDefaults·예산 포함)
        return new StageEndChoices(null, null, newT, fp, fm);
    }
     ...
     */
    /** 강화 벤치마크: 원본보다 작아지면 안 되는 '이득 파라미터'(값이 클수록 강함). cost·turns 등 하방·양날은 제외. */
    private static final java.util.Set<String> UPGRADE_BENEFIT_PARAMS = java.util.Set.of(
        "uses","scope","power","depth","range","choices","count","scale","dice",
        "intensity","trigger_freq","buff_amount","buff_turns");

    private void preserveActiveNature(TraitData upg, TraitData src) {
        if (upg == null || src == null) return;
        // 원본이 시스템 메커니즘(effect_type)이 없으면 능동성만 유지.
        if (src.effectType == null || src.effectType.isBlank()) {
            if (src.active) upg.active = true;
            return;
        }
        // ★원본 메커니즘 유지 + 파라미터 상속★: 강화본이 원본 effect_type을 그대로 쓰게 하고,
        //   원본 effect_params를 '바닥값'으로 상속한다 — 예전엔 성장 JSON에 effect_params가 없으면
        //   applyDefaults가 기본값으로 덮어써 area_scan(scope=3,uses=2) 강화가 scope=2·uses=1로 ★오히려 약화★됐다.
        upg.effectType = src.effectType;
        if (upg.effectParams == null) upg.effectParams = new java.util.HashMap<>();
        if (src.effectParams != null) {
            for (java.util.Map.Entry<String, Integer> en : src.effectParams.entrySet()) {
                String k = en.getKey(); int sv = en.getValue();
                Integer cur = upg.effectParams.get(k);
                if (cur == null) upg.effectParams.put(k, sv);                    // 누락 파라미터는 원본에서 상속
                else if (UPGRADE_BENEFIT_PARAMS.contains(k) && cur < sv)
                    upg.effectParams.put(k, sv);                                 // 이득 파라미터는 원본 미만 금지(강화≥원본)
            }
        }
        // 원본 출신 등급 계승(낮은 출신 실효 보정 유지) — 방어적.
        if ((upg.originGrade == null || upg.originGrade.isEmpty())
                && src.originGrade != null && !src.originGrade.isEmpty())
            upg.originGrade = src.originGrade;
        // 능동 스킬은 강화 후에도 능동으로.
        if (src.active) upg.active = true;
        SystemTraitRegistry.applyDefaults(upg); // 상속·유지한 파라미터 + 새 등급으로 코스트·예산 재계산(클램프 포함)
    }
    ...
    private String computeUpgradeGrade(TraitData t) {
        ...
        return switch (t.grade) {
            case "EX" -> "EX";                 // 이미 최상위 — 유지
            case "S" -> u >= 3 ? "EX" : "S";   // ★S 특성을 깊이 활용(3회+)하고 강화하면 EX 도달(= 2×S 투자: S 확보 + 심화 강화)
            case "A" -> u >= 3 ? "S" : "A";
    ...
    private String normalizeGrade(String g, String fallback) {
        ...
        return switch (g.trim().toUpperCase()) {
            case "EX","S","A","B","C","D","E","F" -> g.trim().toUpperCase();
            default -> (fallback == null || fallback.isEmpty()) ? "C" : fallback;
        };
    }

    /** 등급을 상한(max)으로 클램프. grade가 max보다 높으면 max로 낮춘다. max가 비면 그대로. (max에 "EX"를 주면 EX까지 허용) */
    private String clampGrade(String grade, String max) {
        if (max == null || max.isEmpty()) return grade;
        String[] grades = {"F","E","D","C","B","A","S","EX"};
        int gi = gradeToInt(grade), mi = gradeToInt(max);
        return gi > mi ? grades[Math.max(0, Math.min(grades.length - 1, mi))] : grade;
    }

    /** 등급을 steps 단계 상향 (F<E<D<C<B<A<S, ★상한 S — EX는 여기서 만들지 않는다★). 이미 EX면 EX 그대로 보존(강화 경로 전용). */
    private String bumpGrade(String grade, int steps) {
        if ("EX".equals(grade)) return "EX";   // EX는 최상위 — bump으로 만들지도 내리지도 않는다
        if (steps <= 0) return grade == null ? "C" : grade;
        String[] grades = {"F","E","D","C","B","A","S"}; // ★EX 제외 — gradeBoost 등이 신규 S를 EX로 밀어올리는 인플레 차단
